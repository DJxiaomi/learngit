...
class Order extends IController implements adminAuthorization
{
	...
	public function order_show()
	{
		...
		if($order_id)
		{
			...
			if($data)
			{
		 		...
		 		if ( $data['zuhe_id'] )
		 		{
		 		    $brand_chit_list = brand_chit_zuhe::get_brand_zuhe_list($data['zuhe_id']);
                    $data['brand_chit_list'] = $brand_chit_list;
		 		}
    ...
    public function order_list()
    {
		...
		$user_group = IReq::get('user_group');
		...
		if ( $user_group == 1)
		{
		    $join .= ' left join member as m on o.user_id = m.user_id';
		    $where .= ' and (m.group_id is null or m.group_id < 1)';
		}
