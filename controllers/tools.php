...
class Tools extends IController implements adminAuthorization
{
	...
	function article_list()
	{
	    ...
	    if ( $search['cat_id'] )
	    {
	        $cat_ids = article::get_child_category($search['cat_id']);
	        $where .= ' and ar.category_id in (' . $cat_ids . ')';
	    }
	    
	    if ( $search['keywords'] )
	    {
	        $where .= " and ar.title like '%" . $search['keywords'] . "%'";
	    }
	    
	    // 获取所有分类
	    $cat_list = article_category_class::get_child_category_list(0);
	...
	function article_edit_act()
	{
	    ...
	    $dir = $uploadObj->hashDir();
		$upObj = new IUpload();
		$upObj->setDir($dir);
		$upState = $upObj->execute();
		$status = $upState['jiance_result_upload'][0];
		$jiance_result = IFilter::act(IReq::get('jiance_result','post'));
		if ( $status )
		{
		    if ( $status['flag'] != 1 )
		    {
		        IError::show(403,"上传检测报告失败");
		        exit(); 
		    } else {
		        $jiance_result = $upState['jiance_result_upload'][0]['fileSrc'];
		    }
		}
