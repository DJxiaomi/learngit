...
class countsum_r
{
    	...
    	else
    	{
	    	...
	    	if(isset($buyInfo['goods']['id']) && $buyInfo['goods']['id'])
	    	{
	    		...
	    		foreach($goodsList as $key => $val)
	    		{
	    			...
	    			$groupPrice                = ($val['goods_id'] == 946 || $val['goods_id'] == $this->seller_receipt_id ) ? $buyInfo['goods']['dprice'] : $this->getGroupPrice($val['goods_id'],'goods');
	    	...
	    	if(isset($buyInfo['product']['id']) && $buyInfo['product']['id'])
	    	{
	    		...
	    		foreach($productList as $key => $val)
	    		{
	    			...
	    			$groupPrice                = ($val['goods_id'] == 946 || $val['goods_id'] == $this->seller_receipt_id ) ? $buyInfo['goods']['dprice'] : $this->getGroupPrice($val['product_id'],'product');
	...
	{
	    $_seller_id = IFilter::act(IReq::get('seller_id'),'int');
	    	
	    // 获取商户折扣
	    $seller_discount = seller_discount_level_class::get_seller_discount_by_price($_seller_id,$cprice);
	    ...
	    if ( $goods_list )
	    {
	        foreach( $goods_list as $kk => $vv )
	        {
	            ...
	            if ( $seller_discount )
	            {
	                $goods_list[$kk]['reduce'] = $seller_discount;
	            }
	        }
	    }
     ...
     */
    public function countOrderFee($goodsResult,$province_id,$delivery_id,$payment_id,$is_invoice,$discount = 0,$promo = '',$active_id = '',$dprice = 0, $statement = 1, $stime = 1)
    {
    	//根据商家进行商品分组
    	$sellerGoods = array();
