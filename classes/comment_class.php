<?php
/**
 * 与评论相关的
 *
 * @author walu
 * @packge iwebshop
 */

class Comment_Class
{
	/**
	 * 检测用户是否能够评论
	 *
	 * @param int $comment_id 评论id
	 * @param int $user_id 用户id
	 * @return array() array(成功or失败,数据)
	 */
	public static function can_comment($comment_id,$user_id)
	{
		$comment_id = intval($comment_id);
		$user_id = intval($user_id);

		$tb_comment = new IModel("comment");
		$comment = $tb_comment->getObj("id={$comment_id} AND user_id={$user_id}");
		if(!$comment)
		{
			return "没有这条数据";
		}

		if($comment['status'] != 0)
		{
			return "不能重复评论";
		}
		return $comment;
	}

	/**
	 * 获取某个商品的有关分数的评论数据,根据comment表里面的评价分数做分析
	 *
	 * 获取好评、中评、差评数量及平均分
	 * 返回的值里包含以下几个计算出来的索引
	 *	<ul>
	 *		<li>point_total，总分</li>
	 *		<li>comment_total，评论总数</li>
	 *		<li>average_point，平均分</li>
	 *	</ul>
	 *
	 * @param int $id 商品ID
	 * @return array()
	 */
	public static function get_comment_info($id)
	{
		$data  = array();
		$query = new IQuery("comment");
		$query->fields = "COUNT(*) AS num,point";
		$query->where  = "goods_id = {$id} AND status=1 ";
		$query->group  = "point";

		$config = array(0=>'none',1=>'bad',2=>'middle',3=>'middle',4=>'middle',5=>'good');

		$data['point_grade'] = array('none'=>0,'good'=>0,'middle'=>0,'bad'=>0);
		$data['point_total'] = 0;

		foreach( $query->find() AS $value )
		{
			if($value['point']>=0 && $value['point']<=5)
			{
				$data['point_total']+=$value['point']*$value['num'];
				$data['point_grade'][$config[$value['point']]] += $value['num'];
			}
		}
		$data['comment_total']=array_sum($data['point_grade']);
		$data['average_point']=0;
		if($data['point_total']>0)
		{
			$data['average_point'] = round($data['point_total'] / $data['comment_total']);
		}
		return $data;
	}
	
	public static function get_tutor_seller_point($seller_id = 0)
	{
	    if ( !$seller_id )
	        return false;
	     
	    $comment_db = new IQuery('comment');
	    $comment_db->where = 'seller_id = ' . $seller_id;
	    $comment_list = $comment_db->find();
	     
	    $point = 5;
	    if ( $comment_list )
	    {
	        $point = 0;
	        foreach($comment_list as $kk => $vv)
	        {
	            $point += $vv['point'];
	        }
	         
	        if ( $point )
	            return floor($point / sizeof($comment_list));
	    }
	     
	    return $point;
	}
	
	public static function get_tutor_comment_list($seller_id = 0 )
	{
	    if ( !$seller_id )
	        return false;
	     
	    $comment_db = new IQuery('comment');
	    $comment_db->where = 'seller_id = ' . $seller_id;
	    $comment_list = $comment_db->find();
	     
	    $arr = array(
	        'perfect'  => 0,
	        'good'    => 0,
	        'bad'      => 0,
	        'perfect_list' => array(),
	        'good_list' => array(),
	        'bad_list' => array(),
	    );
	    if ( $comment_list )
	    {
	        foreach( $comment_list as $kk => $vv)
	        {
	            if ( in_array($vv['point'], array(4,5) ))
	            {
	                $arr['perfect']++;
	                $arr['perfect_list'][] = $vv;
	            } else if ( in_array($vv['point'], array(2,3)))
	            {
	                $arr['good']++;
	                $arr['good_list'][] = $vv;
	            } else {
	                $arr['bad']++;
	                $arr['bad_list'][] = $vv;
	            }
	        }
	    }
	    $arr['comment_list'] = $comment_list;
	    return $arr;
	}
	
	public static function get_comment_level_title($point = 0)
	{
	    if ( in_array($point, array(4,5)))
	        return '好评';
	    else if ( in_array($point, array(2,3)))
	        return '中评';
	    else
	        return '差评';
	}
	
	// 获取评论列表
	public static function getCommentsList($goods_id, $num = 3)
	{
	    $query = new IQuery("comment AS a");
	    $query->fields = "a.*,b.username,b.head_ico";
	    $query->join = "left join user AS b ON a.user_id=b.id";
	    $query->where = "a.goods_id='" . $goods_id . "' AND a.status=1 ";
	
	    $query->order    = "a.id DESC";
	    $query->page     = 1;
	    $query->pagesize = $num;
	
	    $comments = $query->find();
	
	    return $comments;
	}
}