...
class Sms_class
{
    ...
    {
        return $this->sendSMS( $this->uid, $this->pwd, $mobile, $content );
    }
    ...
    * @return array
    */
    public function sendSMS($uid,$pwd,$mobile,$content,$template='')
    {
    ...
    //把数组转json字符串
    public function array_to_json($p)
    {
        return urldecode(json_encode(json_urlencode($p)));
    }
    //url转码
    public function json_urlencode($p)
    {
        if( is_array($p) )
        {
    ...
    //把json字符串转数组
    public function json_to_array($p)
    {
     ...
     * 获取短信信息
     */
    public static function get_sms_info( $mobile, $action, $where = '' )
    {
        if ( !$mobile || !$action )
        {
            return false;
        ...
        $sms_db = new IQuery('sms');
        $where = ( $where ) ? "mobile = '$mobile' and action = $action and $where" : "mobile = '$mobile' and action = $action";
        $sms_db->where = $where;
        return $sms_db->getOne();
    }
    ...
    public static function drop_info( $where )
    {
