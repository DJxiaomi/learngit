...
class CountSum_n
{
	...
	public function goodsCount($buyInfo,$promo='',$active_id='')
	{
    	...
    	else
    	{
	    	...
	    	if(isset($buyInfo['product']['id']) && $buyInfo['product']['id'])
	    	{
	    		...
	    		{
	    		    if ( $val['goods_id'] > 0  && $val['product_id'] > 0 )
	    		    {
	    		        // 读取可用代金券信息
	    		        $prop_info = prop_class::get_prop_by_product_id($user_id,$val['goods_id'],$val['product_id']);
	    		        if ( $prop_info )
	    		        {
	    		            $productList[$key]['prop_info'] = $prop_info;
	    		        }
	    		        
	    		        // 使用代金券的操作
	    		        if ( isset($buyInfo['product']['use_chit_list'][$key]) &&  $buyInfo['product']['use_chit_list'][$key] == 1 && $prop_info )
	    		        {
	    		            $value = $prop_info['value'];
	    		            $productList[$key]['reduce'] += $value;
	    		            $productList[$key]['prop_info']['is_use'] = 1;
	    		        }
	    		    }
	    			...
	    			if($buyInfo['product']['data'][$val['product_id']]['count'] <= 0 || $buyInfo['product']['data'][$val['product_id']]['count'] > $val['store_nums'])
	    			{
	    				...
	    				$this->error .= "<货品：".$val['name']."> 购买数量超出库存，请重新调整购买数量。" . $buyInfo['product']['data'][$val['product_id']]['count'] . ':' . $va['store_nums'];
	    			}

	    			$groupPrice                  = $this->getGroupPrice($val['product_id'],'product');
	    			if ( !$productList[$key]['reduce'])
	    			...
	    			$productList[$key]['sum']    = ($current_reduce_all > $current_sum_all ) ? 0 : $current_sum_all - $current_reduce_all;
	    			if(!isset($this->seller[$val['seller_id']]))
	    			{
	    				$this->seller[$val['seller_id']] = 0;
	    			}
	...
	//购物车计算
	public function cart_count($id = '',$type = '',$buy_num = 1,$promo='',$active_id='', $use_chit_list = array())
	{
		//单品购买
		if($id && $type)
		{
    		...
    		$buyInfo = array(
    			$type => array('id' => array($id),'data' => array($id => array('count' => $buy_num)),'count' => $buy_num, 'use_chit_list' => $use_chit_list)
    		);
		}
		else
		{
     ...
     */
    public function countOrderFee($goodsResult,$province_id,$delivery_id,$payment_id,$is_invoice,$discount = 0,$promo = '',$active_id = '', $use_chit_list = array())
    {
    	//根据商家进行商品分组
    	$sellerGoods = array();
    	...
    	foreach($sellerGoods as $seller_id => $item)
    	{
    		...
    		$sellerData = $this->goodsCount($cartObj->cartFormat(array("goods" => $goodsArray,"product" => $productArray, 'use_chit_list' => $use_chit_list)),$promo,$active_id,$use_chit_list);
