...
class goods_class
{
	...
	public function update($id,$postData)
	{
		...
		if($this->seller_id)
		{
			...
			$goodsUpdateData['brand_id'] = $seller_info['brand_id'];
		...
		$goodsUpdateData['img']   = IFilter::act($goodsUpdateData['img']);
		$goodsUpdateData['store_nums']   = IFilter::act(array_sum($postData['_store_nums']),'int');
		$goodsUpdateData['cusval']   = isset($postData['_cusval'][$defaultKey]) ? IFilter::act($postData['_cusval'][$defaultKey]) : '';
		$goodsUpdateData['classnum']   = isset($postData['_classnum'][$defaultKey]) ? IFilter::act($postData['_classnum'][$defaultKey],'int') : 0;
		$goodsUpdateData['school_time']   = isset($postData['_school_time'][$defaultKey]) ? IFilter::act($postData['_school_time'][$defaultKey]) : '';
		$goodsUpdateData['Age_grade']   = isset($postData['_Age_grade'][$defaultKey]) ? IFilter::act($postData['_Age_grade'][$defaultKey]) : '';
		$goodsUpdateData['market_price']   = IFilter::act(array_sum($postData['_market_price']));
		$goodsUpdateData['chit']   = IFilter::act(array_sum(($postData['_chit']) ? $postData['_chit'] : array(0)));
		$goodsUpdateData['max_price']   = IFilter::act(array_sum(($postData['_max_price']) ? $postData['_max_price'] : array(0)));
		$goodsUpdateData['dqk_times']   = IFilter::act(array_sum(($postData['_dqk_times']) ? $postData['_dqk_times'] : array(0)));
		if ( $this->seller_id )
		{
			$goodsUpdateData['seller_id'] = $this->seller_id;
		}
		...
		$seller_info = seller_class::get_seller_info($goodsUpdateData['seller_id']);
		...
		if(isset($postData['_spec_array']))
		{
			...
			foreach($postData['_goods_no'] as $key => $rs)
			{
				$productsData = array(
					...
					'cusval'		=>	IFilter::act($postData['_cusval'][$key]),
					'classnum'		=>	IFilter::act($postData['_classnum'][$key],'int'),
					'school_time'	=>	IFilter::act($postData['_school_time'][$key]),
					'Age_grade'		=>	IFilter::act($postData['_Age_grade'][$key]),
					'chit'			=>	IFilter::act($postData['_chit'][$key]),
					'max_price'		=>	IFilter::act($postData['_max_price'][$key]),
					'dqk_times'		=>	IFilter::act($postData['_dqk_times'][$key]),
				);
				$productsDB->setData($productsData);
				$productIdArray[$key] = $productsDB->add();
								
				// 短期课添加与修改
				$class_num = IFilter::act($postData['_classnum'][$key],'int');
				if ( $productsData['dqk_times'] > 0 )
				{
					$brand_chit_db = new IQuery('brand_chit');
					$brand_chit_db->where = 'seller_id = ' . $goodsUpdateData['seller_id'] . ' and goods_id = ' . $id . ' and product_id = ' . $productIdArray[$key] . ' and category = 2';
					$info = $brand_chit_db->find();
					$info = ($info) ? current($info) : array();
					$max_price = ($productsData['sell_price'] / $class_num * $productsData['dqk_times']);
					
					$brand_chit_db = new IModel('brand_chit');
					// 代金券组合的内容
					$data = array(
						'name'		=>	$goodsUpdateData['name'],
						'seller_id'	=>	$goodsUpdateData['seller_id'],
						'brand_id'	=>	$seller_info['brand_id'],
						'goods_id'	=>	$id,
						'type'		=>	1,
						'product_id'	=>	$productIdArray[$key],
						'use_times'	=>	$productsData['dqk_times'],
						'category'	=>	2,
						'max_price'	=>	$max_price,
					    'tc_price'  =>  $max_price,
						'commission' => 0,
					...
					if ( $info )
					{
						...
						$brand_chit_db->update('id = ' . $info['id']);
					} else {
						$brand_chit_db->setData($data);
						$brand_chit_db->add();
					}
					...
					$brand_chit_db = new IModel('brand_chit');
					$brand_chit_db->del('seller_id = ' . $goodsUpdateData['seller_id'] . ' and goods_id = ' . $id . ' and product_id = ' . $productIdArray[$key] . ' and category = 2');
				}
			}
		} else {
				...
				// 无规格短期课添加与修改
				$class_num = $goodsUpdateData['classnum'];
				if ( $goodsUpdateData['dqk_times'] > 0 )
				{
					$brand_chit_db = new IQuery('brand_chit');
					$brand_chit_db->where = 'seller_id = ' . $goodsUpdateData['seller_id'] . ' and goods_id = ' . $id . ' and product_id = 0 and category = 2';
					$info = $brand_chit_db->find();
					$info = ($info) ? current($info) : array();
					$max_price = ($goodsUpdateData['sell_price'] / $class_num * $goodsUpdateData['dqk_times']);
					...
					$data = array(
						...
						'type'		=>	1,
						'product_id'	=>	0,
						'use_times'	=>	$goodsUpdateData['dqk_times'],
						'category'	=>	2,
						'max_price'	=>	$max_price,
					    'tc_price'  =>  $max_price,
						'commission' => 0,
					...
					$brand_chit_db = new IModel('brand_chit');
					$brand_chit_db->del('seller_id = ' . $goodsUpdateData['seller_id'] . ' and goods_id = ' . $id . ' and product_id = 0 and category = 2');
				}
		...
		//$postData['_imgList'] = ($postData['_imgList']) ? trim($postData['_imgList'],',') : $postData['_imgList'];
		
		if(isset($postData['_imgList']) && $postData['_imgList'] && is_array($postData['_imgList']))
		{
		    $photoDB = new IModel('goods_photo');
		    foreach($postData['_imgList'] as $k => $v )
		    {
		        if(stristr($v,'base64'))
		        {
		            $img_tmp = explode(',',$v);
		    ...
		    $postData['_imgList'] = implode(',', $postData['_imgList']);
		}
		...
		if(isset($postData['_imgList']) && $postData['_imgList'])
		{
			...
			$goodsUpdateData['img'] = $postData['_imgList'][0];
			$goods_db = new IModel('goods');
			$goods_db->setData($goodsUpdateData);
			$goods_db->update('id = ' . $id);
		...
		// 处理课程的搜索关键词
		$search_words = self::get_goods_search_words($id);
		$goods_db2 = new IModel('goods');
		$search_words_data = array(
		    'search_words' => $search_words,
		);
		$goods_db2->setData($search_words_data);
		$goods_db2->update('id = ' . $id);
	...
	*/
	...
	public static function getSearchCondition($search)
	{
		...
		$join[] = 'left join seller as s on go.seller_id = s.id';
		...
		if(isset($search['name']) && isset($search['keywords']))
		{
			...
			if($keywords)
			{
				...
				{
					$sellerDB = new IModel('seller');
					$sellerRow= $sellerDB->getObj('true_name like "%'.$keywords.'%" or shortname like "%'.$keywords.'%"');
	...
	// 获取商家的课程列表
	public static function get_goods_list_by_seller_id( $seller_id = 0, $limit = 10 )
	{
	    if ( !$seller_id )
	        return false;
	    
	    $productModel = new IModel('products');
	    $goods_db = new IQuery('goods');
	    $goods_db->where = 'seller_id = ' . $seller_id . ' and is_del = 0';
	    $goods_db->limit = $limit;
	    //$goods_db->order = 'sale desc';
	    $goods_db->order = 'sort desc';
	    $goods = $goods_db->find();
	    foreach($goods AS $idx => $good)
	    {
	    	$product = $productModel->getObj("goods_id = '$good[id]'", 'min(market_price) AS minprice, max(market_price) AS maxprice');
	    	$goods[$idx]['sell_price'] = $good['sell_price'];
	    	$goods[$idx]['minprice'] = $product['minprice'];
	    	$goods[$idx]['maxprice'] = $product['maxprice'];
	    }
	...
	// 获取品牌的课程列表
	public static function get_goods_list_by_brand_id( $brand_id = 0, $limit = 10 )
	{
	    if ( !$brand_id )
	        return false;
	    ...
	    $productModel = new IModel('products');
	    $goods_db = new IQuery('goods');
	    $goods_db->where = 'brand_id = ' . $brand_id . ' and is_del = 0';
	    $goods_db->limit = $limit;
	    //$goods_db->order = 'sale desc';
	    $goods_db->order = 'sort desc';
	    $goods = $goods_db->find();
	...
	public static function get_goods_info( $goods_id, $fields = '*' )
	{
	    $goods_db = new IQuery('goods');
	    $productDB = new IModel('products');
	    $goods_db->where = 'id = ' . $goods_id . ' and is_del = 0';
	    $goods_db->fields = $fields;
	    $goods = $goods_db->getOne();
	    $price = $productDB->getObj("goods_id = '$goods_id'", 'min(market_price) AS minprice, max(market_price) AS maxprice, id');
	    $products = $productDB->query("goods_id = '$goods_id'");
	    $goods['products'] = $products;
	    $goods['minprice'] = $price['minprice'];
	    $goods['maxprice'] = $price['maxprice'];
	
	    return $goods;
	}
	
	// 获取课程列表
	public static function get_goods_list_by_cats($cat_ids = array())
	{
	    $category_extend_db = new IQuery('category_extend');
	    $category_extend_db->where = db_create_in($cat_ids,'category_id');
	    $category_extend_db->fields = 'distinct(goods_id) as goods_id';
	    $goods_ids = $category_extend_db->find();
	    return $goods_ids;
	}
	
	// 获取课程的搜索关键词
	public static function get_goods_search_words($goods_id = 0)
	{
	    if ( !$goods_id )
	    {
	        return false;
	    ...
	    // 获取课程名称
	    $search_words = array();
	    $search_words[] = $goods_info['name'];
	    
	    // 获取课程分类
	    $category_db->join = 'left join category_extend as ce on c.id = ce.category_id';
	    $category_db->where = 'ce.goods_id = ' . $goods_info['id'];
	    ...
	    // 合并以前存在的关键词、去掉重复/空值
	    $goods_search_words = $goods_info['search_words'];
	    ...
	    return implode(',', $goods_search_words);
	}
}