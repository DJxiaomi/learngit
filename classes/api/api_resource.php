...
return array(
	...
	//根据brand_id获取seller的数据
	'getsellerBrandInfo'=>array(
	    'query'=>array(
		   'name' =>'good as go, seller as s',
		   'where'=>'go.brand_id=#id# and s.brand_id=go.brand_id',
		   'fields'=>'s.content,s.address',
		   'type'=>'row'
	...
	'getCommendNew' => array(
		'query' => array(
			...
			'join' => 'left join goods as go on co.goods_id = go.id left join seller as se on go.brand_id=se.brand_id',
			'where' => 'co.commend_id = 1 and go.is_del = 0 AND go.id is not null',
			'fields' => 'go.img,go.sell_price,go.name,go.id,go.market_price,go.keywords,go.brand_id,se.address,se.shortname',
			'limit'=>'10',
			'order'=>'go.sort asc'
		)
	),
	...
	'getCategoryExtendList'=>array(
	    'query'=>array(
	    	...
	    	'join'  => 'left join goods as go on go.id = ca.goods_id left join seller as se on go.brand_id=se.brand_id',
	    	'where' => 'ca.category_id in(#categroy_id#) and go.is_del = 0',
	    	'order' => 'go.sort asc',
	    	'fields'=> 'go.id,go.name,go.img,go.sell_price,go.keywords,go.market_price,se.address,se.shortname',
	    	'limit' => 10,
	    )
	),
    //根据商品分类取得商品列表2
    'getCategoryExtendList2'=>array(
        'query'=>array(
            'name'  => 'category_extend as ca',
            'join'  => 'left join goods as go on go.id = ca.goods_id join seller as s on go.seller_id = s.id',
            'where' => 'go.is_del = 0 and s.is_authentication = 1 and s.is_system_seller = 0 and ca.category_id in(#categroy_id#) and go.is_del = 0',
            'order' => 'go.sort asc',
    ...
    'getUserPropListing' => array(
        'file' => 'ucenter.php','class' => 'APIUcenter'
    ),
    'getUserPropListed' => array(
        'file' => 'ucenter.php','class' => 'APIUcenter'
    ),
    'getUserPropListNotPay' => array(
        'file' => 'ucenter.php','class' => 'APIUcenter'
    ),
    'getUserPropZuheListing' => array(
        'file' => 'ucenter.php','class' => 'APIUcenter'
    ),
    'getUserPropZuheListNotPay' => array(
        'file' => 'ucenter.php','class' => 'APIUcenter'
    ),
);