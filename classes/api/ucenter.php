...
class APIUcenter
{
    ...
    public function getUserPropList($userid, $type = 1, $page_size = 12)
    {
        $page = IReq::get('page') ? IFilter::act(IReq::get('page'),'int') : 1;
        $query = new IQuery('order as o');
        $where = "o.user_id =" . $userid . " and o.if_del= 0 and o.statement = 2 and o.zuhe_id = 0";
         
        switch ($type)
        {
            case 1:
                $query->where = $where . " and status = 2 and pay_status = 1 and p.is_used = 0 and p.is_close = 0 and p.is_pay = 1";
                $query->join = 'left join prop2 as p on p.id = o.prop';
                $query->fields = 'o.*, p.id as prop_id,p.value as prop_value';
                break;
            case 2:
                $query->where = $where . ' and p.is_used = 1 and p.is_pay = 1';
                $query->join = 'left join prop2 as p on p.id = o.prop';
                $query->fields = 'o.*, p.id as prop_id,p.value as prop_value';
                break;
            case 3:
                $query->where = $where . " and pay_status = 0 and status = 1";
                break;
        }
    ...
    // 已使用的学习券
    public function getUserPropListed($userid)
    {
        $query = new IQuery('order as o');
        $query->where = "o.user_id =" . $userid . " and o.if_del= 0 and o.statement = 2 and o.chit_id > 0 and o.zuhe_id = 0 and p.is_used = 1 and p.is_pay = 1";
        $query->join = 'left join prop2 as p on p.id = o.prop';
        $query->fields = 'o.*';
        $query->order = "id desc";
        $props = $query->find();
    ...
    // 未使用的学习券
    public function getUserPropListing($userid)
    {
        $query = new IQuery('order as o');
        $query->where = "o.user_id =" . $userid . " and o.if_del= 0 and o.statement = 2 and o.chit_id > 0 and o.zuhe_id = 0 and o.status = 2 and o.pay_status = 1 and p.is_used = 0 and p.is_close = 0 and p.is_pay = 1";
        $query->join = 'left join prop2 as p on p.id = o.prop left join order_goods as og on o.id = og.order_id';
        $query->fields = 'o.*,og.verification_code';
        $query->order = "id desc";
        //$query->page  = $page;
        $props = $query->find();
        return $props;
    ...
    // 未支付的学习券
    public function getUserPropListNotPay($userid)
    {
        $query = new IQuery('order as o ');
        $query->where = "o.user_id =" . $userid . " and o.if_del= 0 and o.statement = 2 and o.chit_id > 0 and o.zuhe_id = 0 and pay_status = 0 and status = 1";
        $query->order = "id desc";
        $props = $query->find();
        return $props;
    }
    public function getUserPropZuheListing($userid)
    {
        $query = new IQuery('order as o');
        $query->where = "o.user_id =" . $userid . " and o.if_del= 0 and o.statement = 2 and o.zuhe_id > 0 and o.status = 2 and o.pay_status = 1";
        $query->join = 'left join brand_chit_zuhe as cz on o.zuhe_id = cz.id left join order_goods as og on o.id = og.order_id';
        $query->fields = 'o.*,og.verification_code,cz.name,cz.price,cz.content,cz.start_time,cz.end_time';
        $query->order = "id desc";
        $props = $query->find();
        return $props;
    }
    
    public function getUserPropZuheListNotPay($userid)
    {
        $query = new IQuery('order as o ');
        $query->where = "o.user_id =" . $userid . " and o.if_del= 0 and o.statement = 2 and o.zuhe_id > 0 and o.pay_status = 0 and o.status = 1";
        $query->join = 'left join brand_chit_zuhe as cz on o.zuhe_id = cz.id left join order_goods as og on o.id = og.order_id';
        $query->fields = 'o.*,og.verification_code,cz.name,cz.price,cz.content,cz.start_time,cz.end_time';
        $query->order = "id desc";
        $props = $query->find();
