...
class Order_Class
{
	...
	public static function updateOrderStatus($orderNo,$admin_id = '',$note = '')
	{
		...
		$member_info = member_class::get_member_info($orderRow['user_id']);
		if($orderRow['pay_status'] == 1)
		{
		    ...
		    if ( $orderRow['status'] == 5)
		    {
			    ...
			    // 生成订单日志
			    order_log_class::add_log( $orderRow['id'], self::getOrderStatus($orderRow) );
			    
			    // 处理订单提成
			    self::handle_order_promo_commission($orderRow['id']);
		    }
			return $orderRow['id'];
		}
		else if($orderRow['pay_status'] == 0)
		{
			...
			if($orderRow['user_id'])
			{
				...
				if($memberRow)
				{
			    	...
			    	$awardText = $proObj->setAward($user_id);
			    	if($awardText)
			    	{
			...
			$goods_array = json_decode($goods_array);
			$goods_name = $goods_array->name;
			
			// 发送短信给用户
			if ( $order_info['mobile'] )
			{
			    $notice_mobile = array('17707413901','13873364333');
			    if ( !in_array( $order_info['mobile'], $notice_mobile))
	...
	public static function getSearchCondition($search)
	{
		...
		{
		    $pay_type = IFilter::act($search['pay_type'], 'int');
	...
	// 处理订单提成的流程
    public static function handle_order_promo_commission($order_id)
    {
        $promo_list = self::get_order_promo_commission_detail($order_id);
        $order_info = order_class::get_order_info($order_id);
        if ( !$order_info || !in_array($order_info['statement'],array(1,2,4)) )
        {
            return false;
        }
                    ...
                    if ( $vv['commission'] > 0 )
                    {
                        if( $k == 'user' || $k == 'order')
                        {
                            $config = array(
                                ...
                                'num'      => $vv['commission'], //整形或者浮点，正为增加，负为减少
                                'order_no' => $order_info['order_no'] // drawback类型的log需要这个值
                            );
                            $log->write($config);
                        ...
                        if ( $vv['type'] == 'seller')
                        {
                            Bill_class::add_sale( $vv['user_id'], $vv['commission'] );
                            $data = array(
                                'type'  =>  2,
                                'seller_id' =>  $vv['user_id'],
                            );
                        }

                        $data['commission'] = $vv['commission'];
                        $data['create_time'] = time();
                        $data['order_id'] = $order_id;
                        $data['level'] = $kk + 1;
                        $prom_commission->setData($data);
                        $prom_commission->add();
                    }
                }
            }
    ...
    // 计算订单推广总额
    public static function get_order_promo_commission_count($order_id = 0)
    {
        $commission_count = 0;
        $commission = order_class::get_order_promo_commission_detail($order_id);
       
        if ( $commission['user'] )
        {
            foreach($commission['user'] as $kk => $vv )
            {
        ...
        {
            foreach( $commission['seller'] as $kk => $vv )
            {
                $commission_count += $vv['commission'];
            }
        }
        if ( $commission['order'] )
        {
            foreach( $commission['order'] as $kk => $vv )
            {
                $commission_count += $vv['commission'];
            }
        }
    ...
    // 根据订单获取推广提成清单
    public static function get_order_promo_commission_detail($order_id = 0)
    {
        if ( !$order_id )
            return false;
        
        $order_info = order_class::get_order_info($order_id);
        $promo_list['user'] = promote_class::get_user_promos_list($order_info['user_id']);
        $promo_list['seller'] = promote_class::get_user_promos_list($order_info['seller_id'],2);
        $promo_list['order'] = promote_class::get_order_promos_user($order_id);
       
        $promo_rules = prom_rules_class::get_prom_rules_list();
        if ($promo_rules)
        {
            foreach($promo_rules as $kk => $vv )
            {
                // 如果存在用户相对应的级别推广人
                if ( $promo_list['user'][$vv['level'] - 1] )
                {
                    $promo_list['user'][$vv['level'] - 1]['commission'] = self::get_order_promo_commission($order_info, $vv['promo_type'], $vv['promo_value'], $vv['user_value']);
                }
                
                // 如果存在商户相对应的级别推广人
                if ( $promo_list['seller'][$vv['level'] - 1] )
                {
                    $promo_list['seller'][$vv['level'] - 1]['commission'] = self::get_order_promo_commission($order_info, $vv['promo_type'], $vv['promo_value'], $vv['seller_value']);
                }
                
                // 如果存在订单相对应的级别推广人
                if ( $promo_list['order'][$vv['level'] - 1] )
                {
                    $promo_list['order'][$vv['level'] - 1]['commission'] = self::get_order_promo_commission($order_info, $vv['promo_type'], $vv['promo_value'], $vv['order_value']);
                }
            }
        }
    ...
    // 根据推广规则计算提成,只有statement = 1 和 statement = 2 && chit_id > 0的订单才有提成
    public static function get_order_promo_commission($order_info, $promo_type, $promo_value, $value )
    {
        if ( !$order_info || ( $order_info['statement'] == 2 && $order_info['chit_id'] <= 0 && $order_info['zuhe_id'] <= 0 ) || $order_info['statement'] == 3 || $order_info['statement'] == 4 )
            return 0;
        ...
        switch( $promo_type )
        {
            // 比例提成
            case 1:
                $nums = self::get_order_profit($order_info) * $promo_value / 100;
                break;
            // 按固定值
            case 2:
                $nums = $promo_value;
    ...
    // 计算订单的利润,该方法只使用于statement = 1 和 statement = 2 && chit_id > 0的订单
    public static function get_order_profit($order_info)
    {
        if ( !$order_info || ( $order_info['statement'] == 2 && $order_info['chit_id'] <= 0 && $order_info['zuhe_id'] <= 0 ) || $order_info['statement'] == 3 )
        {
            return false;
        }
        ...
        if ( $order_info['statement'] == 1 )
        {
			...
			if ( $goods_list )
			{
				foreach( $goods_list as $kk => $vv )
				{
                    if ( $vv['product_id'] )
                    {
                        $info = products_class::get_product_info($vv['product_id']);
                    } else {
                        $info = goods_class::get_goods_info($vv['goods_id']);
                    }
                    
                    if ( $info['max_price'] > 0 )
                    {
                        $order_profit += $info['max_price'] * $vv['goods_nums'];
                    } else {
                        $seller_info = seller_class::get_seller_info($vv['seller_id']);
                        $commission_fee = $seller_info['commission'];
                        $order_profit += $vv['goods_price'] * ($commission_fee / 100);
                    }
				}
			}
			return $order_profit;
            ...
            {
        	    // 获取商户折扣
        	    $seller_discount = seller_discount_level_class::get_seller_discount_by_price($order_info['seller_id'],$order_info['payable_amount'],2);
        	    return $seller_discount['profit'] - $seller_discount['discount'];
            } else {
                $seller_info = seller_class::get_seller_info($order_info['seller_id']);
                $commission_fee = $seller_info['commission'];
                return $order_info['order_amount'] * ($commission_fee / 100);
            }
        } else {
			// 购买代金券
			if ($order_info['chit_id'] > 0 )
			{
				$chit_info = brand_chit_class::get_chit_info($order_info['chit_id']);
				return $order_info['order_amount'] - $chit_info['commission'];
			} else if ( $order_info['zuhe_id'] > 0 )
			{
				// 购买代金券组合
				$brand_chit_db = new IQuery('brand_chit as bc');
				$brand_chit_db->fields = 'bc.*';
				$brand_chit_db->join  = 'left join brand_chit_zuhe_detail as bcd on bc.id = bcd.brand_chit_id';
				$brand_chit_db->where = 'bcd.zuhe_id = ' . $order_info['zuhe_id'];
        ...
        return ( !$discount ) ? 0 : $order_info['order_amount'] * $discount / 100 / 5;
    }
    
    // 获取用户的消费次数
    public static function get_user_pay_count($user_id = 0)
    {
        if ( !$user_id )
            return 0;
        
        $order_db = new IQuery('order');
        $order_db->fields = 'count(*) as count';
        $order_db->where = 'user_id = ' . $user_id . ' and pay_status = 1 and order_amount > 0 ';
        $order_info = $order_db->getOne();
        return $order_info['count'];
    }
    
    // 获取商户的交易次数
    public static function get_seller_order_count($seller_id = 0 )
    {
        if ( !$seller_id )
            return 0;
        ...
        $order_db = new IQuery('order');
        $order_db->fields = 'count(*) as count';
        $order_db->where = 'seller_id = ' . $seller_id . ' and pay_status = 1 and order_amount > 0';
        $order_info = $order_db->getOne();
        return $order_info['count'];
    }
     ...
     * 获取订单信息
     * @param int 订单ID
     * @param int 参数类型, 1为order_id, 2为order_no
     * @param string 字段名
     * @return array
     */
    public static function get_order_info( $order_id , $type = 1, $fields = '*' )
    {
        if ( !$order_id )
        {
            return array();
        ...
        $order_db = new IQuery('order');
        $order_db->where = ( $type == 1 ) ? 'id = ' . $order_id : "order_no =  '$order_id'";
        $order_db->fields = $fields;
        return $order_db->getOne();
    }
    
    // 根据类型获取不同状态的订单列表
    public static function get_order_list($user_id = 0, $type = 1, $page = 1, $page_size = 10)
    {
        if ( !$user_id )
            return false;
        ...
        switch($type)
        {
            // 已付款
            case 1:
                $order_db->where = $where . ' and status = 2 and pay_status =  1';
                break;
                // 未付款
            case 2:
                $order_db->where = $where . ' and status = 1 and pay_status =  0';
                break;
                // 已完成
            case 3:
                $order_db->where = $where . ' and status = 5 and pay_status =  1';
                break;
                // 已取消
            case 4:
                $order_db->where = $where . ' and status = 3 and pay_status =  0';
                break;
        }
        $order_db->page = $page;
        ...
        $result['page_info'] = $order_db->getPageBar();
    
        $paging = $order_db->paging;
        $result['page_count'] = $paging->totalpage;
        return $result;
    }
    ...
    public static function get_order_goods_list($order_id)
    {
        ...
        {
            $orderGoodsList[$kk]['goods_array'] = json_decode($good['goods_array'] );
        }
    ...
    /**
     * 获取课程最高的订单推广提成
     * @param number $goods_id/$brand_id
     * @param number $type 1为正式课，2为短期课和代金券
     */
    public static function get_goods_promote($goods_id = 0, $type = 1)
    {
        if ( !$goods_id )
        {
            return false;
        ...
        if ($type == 1)
        {
            ...
            $product_list = products_class::get_product_list($goods_id);
            if ( !$product_list )
            {
                if ( $goods_info['max_price'] > 0 )
                {
                    $profit[] = $goods_info['max_price'];
                } else {
                    $profit[] = $goods_info['sell_price'] * ($seller_info['commission'] / 100);
                }
            } else {
                foreach($product_list as $kk => $vv )
                {
                    if ( $vv['max_price'] > 0 )
                    {
                        ...
                        $profit[] = $vv['sell_price'] * ($seller_info['commission'] / 100);
                    }
                }
            }
        } else {
            // 代金券或者短期课
            $brand_chit_info = brand_chit_class::get_chit_info($goods_id);
            $profit[] = $brand_chit_info['max_price'] - $brand_chit_info['commission'];
        }
        ...
        // 提成类别
        switch( $promo_rules[0]['promo_type'] )
        {
            // 比例提成
            case 1:
                $nums = $profit * $promo_rules[0]['promo_value'] / 100;
                break;
                // 按固定值
            case 2:
                $nums = $promo_rules[0]['promo_value'];
        }
