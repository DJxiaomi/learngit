    ...
    public static function countSellerOrderFee($orderList)
    {
    	...
    	if($orderList && is_array($orderList))
    	{
    		...
    		foreach($orderList as $key => $item)
    		{
    			...
    			// 根据订单利润计算出，返还商户的金额
    			// 全款订单和商家收款订单
    			if ( in_array($item['statement'], array(1,4)) )
    			{
    			    $order_info = order_class::get_order_info($item['id']);
    			    $order_profit = order_class::get_order_profit($order_info);
                    $result['settled'] = ($order_profit > 0 ) ? $order_info['order_amount'] - $order_profit : $order_info['order_amount'];
    			} else if ( in_array($item['statement'], array(2)) ) {
    			    // 购买代金券的订单
    			    $order_info = order_class::get_order_info($item['id']);
    			    $order_profit = order_class::get_order_profit($order_info);
    			    $result['settled'] = $order_info['order_amount'] - $order_profit;
    			}
    		}
    	}
