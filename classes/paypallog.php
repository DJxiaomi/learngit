 ...
 * @copyright (c) 2015 www.dsanke.com
...

{
...

	{
		...
		$finnalAmount = $this->user['paypal'] + $this->amount;
		...
		$memberDB->setData(array("paypal" => $finnalAmount));

		$isChpaypal = $memberDB->update("user_id = ".$this->user['id']);
		...
		$tb_paypal_log = new IModel("paypal_log");
		...
		$tb_paypal_log->setData($insertData);

		$result = $tb_paypal_log->add();
