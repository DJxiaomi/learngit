 ...
 * @brief 购物车类库
 */
class Cart_t extends IInterceptorBase
{
	/*购物车简单cookie存储结构
		...
		if($goodsIdArray)
		{
			...
			foreach($result['product']['data'] as $key => $val)
			{
				if(isset($goodsArray[$val['goods_id']]))
				{
					...
					//购物车中的金额累加
					    $result['product']['data'][$key]['sell_price'] = $cartValue['dprice'];
					    $result['product']['dprice'] = $cartValue['dprice'];
					    $result['product']['statement'] = $cartValue['statement'];
					    $result['product']['stime'] = $cartValue['stime'];
					$result['sum']   += $result['product']['data'][$key]['sell_price'] * $val['count'];
				}
			}
		}
		$result['statement'] = $cartValue['statement'];
		$result['use_coupon'] = $cartValue['use_coupon'];
		$result['coupon_nums'] = $cartValue['coupon_nums'];
		$result['statement'] = $cartValue['statement'];
		return $result;
	}
