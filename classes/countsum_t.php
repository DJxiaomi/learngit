	...
	public function goodsCount($buyInfo,$promo='',$active_id='')
	{
    	...
    	if($promo && $active_id)
    	{
	    	...
	    	if($activeResult === true)
	    	{
				...
				$typeRow['reduce'] = $typeRow['market_price'] - $disPrice;
				$typeRow['count']  = $ac_buy_num;
    			$current_sum_all   = $typeRow['market_price'] * $ac_buy_num;
    	...
    	else
    	{
	    	...
	    	if(isset($buyInfo['goods']['id']) && $buyInfo['goods']['id'])
	    	{
	    		...
	    		$goodsList  = $goodsObj->query('go.id in ('.$goodsIdStr.')','go.name,go.cost_price,go.market_price,go.id as goods_id,go.img,go.sell_price,go.dprice,go.rprice,go.point,go.weight,go.store_nums,go.exp,go.goods_no,0 as product_id,go.seller_id');
	    		
	    		// 在修改sell_price之前保存sell_price的值存入数据库
	    		// 修改价格, update by jack 20161208
	    		$goodsList = $this->_update_goods_list($goodsList, $buyInfo['goods']['dprice'], $statement, $buyInfo['use_coupon'], $buyInfo['coupon_nums'] );
	    		...
	    		foreach($goodsList as $key => $val)
	    		{
	    			...
	    			{
	    				$goodsList[$key]['name'] .= "【数量不正确】";
	    				$this->error .= "<商品：".$val['name']."> 购买数量不正确，请重新调整购买数量。";
	    			}

	    			$groupPrice                = $this->getGroupPrice($val['goods_id'],'goods');
	    			if ( $statement == 2 || $statement == 3 || $statement == 4 || $statement == 5)
	    			{
	    			    $price = $buyInfo['goods']['dprice'];
	    			} else {
	    			    if ( $buyInfo['use_coupon'] == 1 && $buyInfo['coupon_nums'] > 0 )
	    			        $price = $val['market_price'];
	    			    else 
	    			       $price = $val['market_price'];
	    			}
	    			$goodsList[$key]['reduce'] = $groupPrice === null ? 0 : $price - $groupPrice;
	    			$goodsList[$key]['count']  = $buyInfo['goods']['data'][$val['goods_id']]['count'];
	    			$current_sum_all             = $price * $goodsList[$key]['count'];
	    			$goodsList[$key]['sum']    += $current_sum_all;
	    			if(!isset($this->seller[$val['seller_id']]))
	    			{
	    				$this->seller[$val['seller_id']] = 0;
	    			}
	    			$this->seller[$val['seller_id']] += $goodsList[$key]['sum'];
	    			$goodsList[$key]['point']  += $current_sum_all;

	    			//全局统计
			    	$this->weight += $val['weight'] * $goodsList[$key]['count'];
			    	//$this->point  += $val['point']  * $goodsList[$key]['count'];
			    	$this->point  += $current_sum_all;
			    	$this->exp    += $val['exp']    * $goodsList[$key]['count'];
			    	$this->sum    += $current_sum_all;
			    	$market_sum += $val['market_price'] * $goodsList[$key]['count'];
			    	$this->reduce += $current_reduce_all;
	    	...
	    	if(isset($buyInfo['product']['id']) && $buyInfo['product']['id'] )
	    	{
	    		...
	    		$productObj->fields = 'pro.sell_price,pro.cost_price,pro.market_price,pro.weight,pro.id as product_id,pro.dprice,pro.rprice,pro.spec_array,pro.goods_id,pro.cusname,pro.cusval,pro.classnum, pro.month, pro.student,pro.products_no as goods_no,go.name,go.point,go.exp,go.img,go.seller_id';
	    		$productList  = $productObj->find();
	    		
	    		// 在修改sell_price之前保存sell_price的值存入数据库
	    		// 修改价格, update by jack 20161208
	    		$productList = $this->_update_goods_list($productList, $buyInfo['product']['dprice'], $statement, $buyInfo['use_coupon'], $buyInfo['coupon_nums'] );

	    		//开始优惠情况判断
	    		foreach($productList as $key => $val)
	    		{
	    			//检查库存
	    			if($buyInfo['product']['data'][$val['product_id']]['count'] <= 0 || $buyInfo['product']['data'][$val['product_id']]['count'] > $val['student'])
	    			{
	    			...
	    			$productList[$key]['reduce'] = $groupPrice === null ? 0 : $price - $groupPrice;
	    			$productList[$key]['count']  = $buyInfo['product']['data'][$val['product_id']]['count'];
	    			$current_sum_all             = $price * $productList[$key]['count'];
	    			$productList[$key]['sum']    += $current_sum_all;
	    			//$productList[$key]['spec']     = order_class::get_goods_spec( $val['cusname'], $val['cusval'], $val['classnum'], $val['month'] );
	    			if(!isset($this->seller[$val['seller_id']]))
	    			{
	    				$this->seller[$val['seller_id']] = 0;
	    			}
	    			$this->seller[$val['seller_id']] += $productList[$key]['sum'];
	    			$productList[$key]['point']  += $current_sum_all;
			    	...
			    	$this->sum    += $current_sum_all;
			    	$market_sum += $val['market_price'] * $productList[$key]['count'];
			    	$this->reduce += $current_reduce_all;
...
//     	$user_prop_count = prop_class::get_user_prop_count( $user_id );
//     	$max_prop_info = order_class::get_max_prop_info( $user_id, $resultList, $statement, $buyInfo['use_coupon'], $buyInfo['coupon_nums'] );
// 	if( $statement != 5 ){
//     		$resultList = $max_prop_info['goods_list'];
// 	}
//     	if ( $statement == 4 )
//     	{
//     	    $resultList      = array_merge($goodsList,$productList);
//     	}
    	...
    	return array(
    		...
    		'seller'     => $this->seller,
    	    'max_prop_info'    =>  $max_prop_info,
    	    'user_prop_count'  =>  $user_prop_count,
    	    'market_sum'   => $market_sum,
    	);
	}
	
	// 修改价格 add by jack 20161208
	public function _update_goods_list($goods_list, $cprice, $statement = 1, $use_coupon, $coupon_nums )
	{
	    if ( $goods_list )
	    {
	        foreach( $goods_list as $kk => $vv )
	        {
	            $sell_price_t = $vv['sell_price'];
	            $goods_list[$kk]['sell_price_t'] = $sell_price_t;
	        }
	    }
	    ...
	    if ( isset( $statement ) && $statement > 0 )
	    {
	        foreach( $goods_list as $kk => $vv )
	        {
	            $sell_price_t = $vv['sell_price'];
	            $goods_list[$kk]['sell_price_t'] = $sell_price_t;
	            //$goods_list[$kk]['dprice'] = order_class::get_dprice_by_cprice( $cprice );
	            //$goods_list[$kk]['rprice'] = order_class::get_rprice_by_cprice( $cprice, $goods_list[$kk]['dprice'] );
	            $goods_list[$kk]['sell_price'] = $cprice;
	        }
	    }
	...
	//购物车计算
	public function cart_count($id = '',$type = '',$buy_num = 1,$promo='',$active_id='', $dprice = 0, $statement = 1, $stime = 1, $use_coupon = 0, $coupon_nums = 0 )
	{
		//单品购买
		if($id && $type)
		{
    		...
    		// 将自定义的dprice加入到数组内
    		$buyInfo[$type]['dprice'] = $dprice;
    		$buyInfo[$type]['stime'] = $stime;
		}
		else
		{
			//获取购物车中的商品和货品信息
     ...
     */
    public function countOrderFee($goodsResult,$province_id,$delivery_id,$payment_id,$is_invoice,$discount = 0,$promo = '',$active_id = '', $dprice = 0, $statement = 1, $stime = 1, $use_coupon = 0, $coupon_nums = 0 )
    {
    	//根据商家进行商品分组
    	$sellerGoods = array();
    	...
    	foreach($sellerGoods as $seller_id => $item)
    	{
    		...
    		$arr = array("goods" => $goodsArray,"product" => $productArray, 'dprice' => $dprice, 'stime' => $stime );
    		if ( $statement == 1 || $statement == 3 || $statement == 5)
    		{
		          $arr['use_coupon'] = $use_coupon;
		          $arr['coupon_nums'] = $coupon_nums;
    		}
    		$arr['statement'] = $statement;
    		$sellerData = $this->goodsCount($cartObj->cartFormat($arr),$promo,$active_id);
    ...
    {
    	$where  = "status in (5,7) and statement != 4 and pay_type != 0 and pay_status = 1 and distribution_status in (1,2)";
     ...
     */
    public static function getAutoSellerGoodsFeeQuery( $seller_id, $order_id )
    {
        ...
        $is_checkout = 0;
        $where  = "id = $order_id and status in (5,7) and pay_type != 0 and pay_status = 1 and distribution_status in (1,2)";
        $where .= " and is_checkout = ".$is_checkout;
    ...
    public static function countSellerOrderFee($orderList)
    {
    	...
    	if($orderList && is_array($orderList))
    	{
    		...
    		foreach($orderList as $key => $item)
    		{
    			...
    			if($item['prop'])
    			{
    			    $prop_ids = explode(',', $item['prop']);
    				$propRow = $propObj->getObj(db_create_in($prop_ids, 'id').' and type = 0');
    			...
    			// 获取商品列表、并计算出商品的成本价格
    			if ( in_array($item['statement'], array(1, 2, 3)) )
    			{
    			    $goods_list = Order_goods_class::get_order_goods_list($item['id']);
    			    if ( $goods_list )
    			    {
    			        $settled = 0;
    			        $not_settled = 0;
    			        foreach( $goods_list as $kk => $vv )
    			        {
    			            ...
    			            if ( $item['statement'] == 1 )
    			            {
    			                $result['cost_price'] += $vv['cost_price'] * $vv['goods_nums'] * 0.9;
    			            }
    			            // 定金订单
    			            else 
    			            {
    			                if ( $item['coupon_nums'] <= 0 )
    			                    $result['cost_price'] += ( $vv['cost_price'] * $vv['goods_nums'] ) - $item['order_amount'];
    			                else
    			                  $result['cost_price'] += ( $vv['cost_price'] * $vv['goods_nums'] ) - $item['rest_price'];
    			            }
    			            **/
    			            ...
    			            if ( $item['statement'] == 1 )
    			            {
    			                $all_count = $vv['cost_price'] * $vv['goods_nums'];
    			                $settled_part = $all_count * 0.9;
    			                $not_settled_part = $all_count - $settled_part;
    			                $settled += $settled_part;
    			                $not_settled += $not_settled_part;
    			            // 学习券订单
    			            	...
    			            	if($item['chit_id'] > 0)
    			            	{
    			            		$chit = brand_chit_class::get_chit_info($item['chit_id']);
    			            		if( ( $chit['max_price'] - $chit['commission'] ) > 0 )
    			            		{
    			            		    $settled += $chit['max_price'] - $chit['commission'];
    			            		} else if ( $item['order_amount'] > 0 && ($chit['max_price'] - $chit['commission']) == 0) {
    			            		    $settled += $item['order_amount'] * 0.01;
    			            		    $settled = $settled * 100;
    			                ...
    			                {
    			                	$settled = 0;
    			                	$not_settled += ( $vv['cost_price'] * $vv['goods_nums'] ) - $item['rest_price'];
    			                }
    			            // 定金订单
    			            } else if ( $item['statement'] == 3 ) {
    			                /**
    			                if ( $item['coupon_nums'] <= 0 )
    			                    $all_count = ( $vv['cost_price'] * $vv['goods_nums'] ) - $item['order_amount'];
    			                else 
    			                    $all_count = ( $vv['cost_price'] * $vv['goods_nums'] ) - $item['rest_price'];
    			                **/
    			                $all_count = ( $vv['cost_price'] * $vv['goods_nums'] ) - $item['rest_price'];
    			                
    			                $settled = 0;
    			                $not_settled += $all_count;
    			            }
    			        }
    			        
    			        $result['cost_price'] = $settled + $not_settled;
    			        $result['settled'] = $settled;
    			        $result['not_settled'] = $not_settled;
    			    }
    			}
    		}
    	}
		...
		//最终结算金额, 修改结算流程，按照商品成本价结算给商家
		//$result['countFee'] = $result['orgCountFee'] - $result['commission'];
		$result['countFee'] = $result['cost_price'];
