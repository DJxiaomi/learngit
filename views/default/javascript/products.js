				...
				$.getJSON(creatUrl("site/getProduct"),{"goods_id":_self.goods_id,"specJSON":specJSON,"random":Math.random},function(json){
					if(json.flag == 'success')
					{
					  ...
					  $.getJSON(creatUrl('site/get_chit_info'),{'goods_id':_self.goods_id,'spec_id':json.data.id},function(content){
						if(content){
							$('.chit_info').html('<p>本课程可用第三课代金券抵<span class="ticket_prop">' + content.max_price + '</span>现金，享受折上折优惠！<a href="' + creatUrl('/simple/cart2d/id/7151/num/1/type/product/statement/2/chitid/' + content.id) + '" class="receive">点我领券</a></p>');
						}
					  })
					}
					else
