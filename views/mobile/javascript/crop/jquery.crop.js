...
(function($){
	$.fn.crop = function(options){
		...
		//如果盒子高度少于宽度提醒设置错误
		if(o.h<o.w){
			mui.toast('容器区域高度须大于等于宽度！');
			return false;
		}
		//如果半径大于盒子宽度的一半，则提醒，用在手机上加限制
		if(o.r>o.w*0.5){
			mui.toast('请在手机上浏览或将浏览器宽度调整到高度二分之一以下！');
			return false;
		}
		var box = $(this);
		if(!box.attr('id')){
		...
		image.onload = function(){
			...
			if(box.html()!=''){
				...
				box.after('<div style="height:'+v.barHeight+'px;width:100%;position:absolute;bottom:0px;"><span style="display:block;position:absolute;left:15px;bottom:15px;color:#fff;z-index:12;background:#f89119;width:60px;height:30px;line-height:30px;text-align:center;border-radius:3px;" id="select_btn">选择</span><span style="display:block;position:absolute;right:15px;bottom:15px;color:#fff;z-index:12;background:#35b345;width:60px;height:30px;line-height:30px;text-align:center;border-radius:3px;" id="edit_finished">完成</span><input type="file" id="selectImageFile" accept="image/*" capture="camera" style="height:0px;width:0px;border:0px;display:none;" /></div>');
			...
			function resetImg(source_img){
				if(source_img.naturalWidth<o.r*2 || source_img.naturalHeight<o.r*2){
					mui.toast('图片尺寸太小，请重新选择不小于'+o.r*2+'*'+o.r*2+'像素大小的图片');
				}else{
					var quality = 1;					
