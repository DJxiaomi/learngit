  ...
  $('.receive').on('click',function(){
      ...
      if ( _spec_id == 0)
      {
        if ( _goods_list[_goods_id]['product_list'].length == 0 && _goods_list[_goods_id]['chit'] > 0 && _goods_list[_goods_id]['max_price'] > 0 )
        {
          ...
          _seller_id = _goods_list[_goods_id]['seller_id'];
        }
      } else {
  ...
  else if ( _spec_list.length > 0 )
  {
    ...
    for( j in _spec_list )
    {
      ...
      if ( _spec_id > 0 && _spec_list[j]['id'] == _spec_id )
      {
        min = price;
        max = price;
        if ( parseFloat(_spec_list[j]['chit']) > 0 )
        {
          _is_buy_chit = true;
        }
    ...
    if ( parseFloat(_goods_list[_goods_id]['chit']) > 0 )
    {
      _is_buy_chit = true;
    }
  }
...
function set_price(str)
{
  if (str == _hide_price )
  {
    console.log(_hide_price_str);
    $('.goodsprice').html('<i>' + _hide_price_str + '</i>');
    $('.goods_price i').html(_hide_price_str);
    $('.shop-nav-bot').hide();
    $('.pop_contents .pay_btn').hide();
  } else {
    $('.goodsprice i').html(str);
    $('.pop_contents .goods_price i').html(str);
    $('.shop-nav-bot').show();
    $('.pop_contents .pay_btn').show();
  }
...
function buy_chit()
{
  ...
  var type      = ( _spec_id > 0 ) ? 'product' : 'goods';
  var goods_id  = (type == 'product') ? _spec_id : _goods_id;
