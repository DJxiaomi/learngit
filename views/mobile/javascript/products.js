...
function productClass(goods_id,user_id,promo,active_id)
{
	...
	this.initSpec = function()
	{
		...
		$('[specId]').bind('click',function()
		{
			...
			if(_self.checkSpecSelected() == true)
			{
				...
				//获取货品数据并进行渲染
				var specData = $(this).data('specData');
				var spec_id = specData.id;
				$.getJSON(creatUrl("site/getProduct"),{"goods_id":_self.goods_id,"product_id":spec_id,"specJSON":specJSON,"random":Math.random},function(json){
					if(json.flag == 'success')
					{
						//货品数据渲染
					  ...
					  $.getJSON(creatUrl('site/get_chit_info'),{'goods_id':_self.goods_id,'spec_id':json.data.id},function(content){
							if(parseFloat(content.max_price) > 0 )
							{
								$('.chit_info').html('<p>本课程可用第三课代金券抵<span class="ticket_prop">' + content.max_order_chit + '</span>现金，享受折上折优惠！<a href="' + creatUrl('/simple/cart2d/id/5280/num/1/type/product/chitid/' + content.id) + '" class="receive">点我领券</a></p>');
							}
					  })
					}
					else
	...
	this.buyNow = function()
	{
		...
		//普通购买
		var url = "/simple/cart2n/id/"+id+"/num/"+buyNums+"/type/"+type;
