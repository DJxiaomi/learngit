...
function orderFormClass()
{
	 ...
	 */
	this.addressInit = function(defaultAddressId, true_name, telephone)
	{
		if(defaultAddressId > 0)
		{
			this.addressActiveId = defaultAddressId;
			$('input:radio[name="radio_address"][value="'+defaultAddressId+'"]').trigger('click');
			//var jsonData = eval('(' + $('input:radio[name="radio_address"][value="'+defaultAddressId+'"]').attr('data-json') + ')');
		...
		} else if(true_name != '' && telephone != '') {
			var jsonData = {"accept_name":true_name,"telphone":telephone,"mobile":telephone,"is_default":"1"};
			this.addressSelected(jsonData);
			//var showTableHtml = template.render('addressShowTemplate',jsonData);
			//$('#addressShowBox').html(showTableHtml);
			//this.addressMod = 'edit';
			//this.addressModToggle();
		} else {
			if($('input:radio[name="radio_address"]').length > 1)
			{
				$('input:radio[name="radio_address"]').eq(0).trigger('click');
			}
			else
			{
				//this.addressMod = 'edit';
				//$('#'+this.addressToggleButton).text('[退出]');
				//$('#address_show_box').slideUp();

				//修改模式
				//$('#address_often').slideDown();
				//$('#address_form').slideDown();
				//$('#address_save_button').slideDown();
	...
	this.messageModToggle = function()
	{
		...
		switch(toggleMod)
		{
			case "edit":
			{
				$('#'+this.messageToggleButton).text('[取消]');
			}
			break;
			...
			case "exit":
			{
				...
				$('#'+this.messageToggleButton).text('[留言]');
			}
			break;
		}
