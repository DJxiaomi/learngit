...
class ITag
{
	...
	{
		return preg_replace_callback('/{(\/?)(\$|url|webroot|theme|skin|rs|echo|query|widget|foreach|set|include|require|if|elseif|else|while|for|js|rs|minify|template)\s*(:?)([^}]*)}/i', array($this,'translate'), $str);
	}
    /**
     * @brief 处理设定的每一个标签
     * @param array $matches
	...
	public function translate($matches)
	{
		if($matches[1]!=='/')
		{
			switch($matches[2].$matches[3])
			{
                ...
                case 'rs:'; return '<?php echo IUrl::creatUrl("")."resource'.$matches[4].'";?>';
				...
				case 'minify:':
				    {
				        if(preg_match('/WEBROOT/', $matches[4]))
				        {
				            $matches[4] = str_replace('WEBROOT', '". rtrim(IUrl::creatUrl(""), "/")."', $matches[4]);
				        }
				        if(preg_match('/RS/', $matches[4]))
				        {
				            $matches[4] = str_replace('RS', '". rtrim(IUrl::creatUrl(""), "/")."/resource', $matches[4]);
				        }
				        if(preg_match('/SKIN/', $matches[4]))
				        {
				            $matches[4] = str_replace('SKIN', '". rtrim($this->getWebSkinPath() , "/")."', $matches[4]);
				        }
				        if(preg_match('/THEME/', $matches[4]))
				        {
				            $matches[4] = str_replace('THEME', '". rtrim($this->getWebViewPath(), "/")."', $matches[4]);
				        }
				        return '<?php echo IUrl::creatUrl("")."resource/min/?f='.$matches[4].'";?>';
				    }
				case 'template:':
				    $file = trim( $matches[4], "'");
				    $file_path = IWEB_ROOT . $file;
				    if ( file_exists( $file_path ))
				    {
				        return '<?php include( IWEB_ROOT . \'' . $file . '\' ); ?>';
				    }
				    else
				        return '';
				default:
