        ...
        <ul class="mui-table-view">
            <?php if($detail_list){?>
              ...
              <li class="mui-table-view-cell mui-media">
                  ...
                  <div class="ptime"><?php if($item['availeble_use_times'] > 0 ){?><?php echo floor($item['availeble_use_times'] / $item['each_times']);?>次<?php echo isset($item['availeble_use_times'])?$item['availeble_use_times']:"";?>课时<?php }else{?>0课时<?php }?></div>
                  <div class="ptime"><?php if($item['availeble_use_times'] > 0){?><a href="javascript:void(0);" class="confirm_dqk" id="<?php echo isset($item['use_list_id'])?$item['use_list_id']:"";?>">确认使用</a><?php }else{?>已完成<?php }?></div>
              </li>
              <?php }?>
            <?php }?>
...
<div class="mui-card order_log">
    <div class="mui-card-content">
        <ul class="mui-table-view">
            ...
            <?php foreach($this->orderStep as $eventTime => $stepData){?>
...
var _update_dqk_ajax = '<?php echo IUrl::creatUrl("/ucenter/update_dqk_status");?>';
$(document).ready(function(){
    $('.confirm_dqk').click(function(){
      ...
      mui.confirm('您确定已经使用了短期课吗？', '提示信息', btnArray, function(e) {
          if (e.index == 1)
          {
            location.href = '<?php echo IUrl::creatUrl("/ucenter/redpacket_use/id/".$this->order_info[order_id]."");?>/use_id/' + use_id;
          }
      });
  	});
})
