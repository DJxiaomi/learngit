...
<div class="mui-content">
      ...
      <?php foreach($manual_list as $key => $item){?>
        ...
        <li class="mui-table-view-cell mui-media last">
          <div class="mui-media-footer">
            ...
            <div class="btns">
              <a href="<?php echo IUrl::creatUrl("/ucenter/manual_use_list/id/".$item['id']."");?>">查看使用记录</a>
              <a href="javascript:void(0);" onclick="active_category('<?php echo isset($item['id'])?$item['id']:"";?>','<?php echo isset($item['category_id'])?$item['category_id']:"";?>')" style="display:none;">扩展分类</a>
            </div>
          </div>
        </li>
        <?php }?>
  ...
  <?php if(sizeof($unactivation)){?>
  <div class="mui-table-view mui-table-notice">
    <a href="<?php echo IUrl::creatUrl("/site/manual_activation/code/".$unactivation['0']['activation_code']."");?>">您还有<?php echo sizeof($unactivation);?>本手册可以激活</a>
  </div>
  <?php }?>
</div>

<div class="choose_category">
  <div class="choose_category_content">
