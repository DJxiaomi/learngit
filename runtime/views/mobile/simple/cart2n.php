...
<form action='<?php echo IUrl::creatUrl("/simple/cart3n");?>' method='post' name='order_form' id="order_form">
	...
	<div class="mui-card">
	    ...
	    <div class="mui-input-row mui-radio mui-left">
			...
			<?php if($this->gid == 1956){?>
			<div class="mui-input-row mui-left">
				<input type="text" name="address" placeholder="收货地址" value="">
			</div>
			<?php }?>
        ...
        <input type="hidden" name="area" value="111111">
				<?php if($this->gid != 1956){?>
				<input type="hidden" name="address" value="收货地址">
				<?php }?>
        ...
        <input type="hidden" name="zip" value="000000">
				...
				<input type="hidden" name="manual_id" value="<?php echo $this->manual_id;?>" />
			</div>
		</div>
	</div>
	...
	<h5 class="mui-content-padded">配送方式</h5>
		...
		<ul class="mui-card">
		    ...
		    <li class="mui-table-view-cell mui-media">
	    		...
	    		<div class="mui-media-body">
					...
					<div class="qindanjg">价格：<span class="red3">&yen;<?php echo isset($item['sell_price'])?$item['sell_price']:"";?></span></div>
					<div>人数：<span class="red3"><?php echo isset($item['count'])?$item['count']:"";?>&nbsp;人</span></div>
					<div>代金券：<?php if($item['prop_info']['value'] > 0){?><input type="checkbox" name="use_chit_list[]" value="1" />使用<?php echo isset($item['prop_info']['value'])?$item['prop_info']['value']:"";?>元<?php }else{?>0<?php }?></div>
	    		</div>
		    </li>
		    <?php }?>
		</ul>
...
function submitForm(){
	...
	<?php if($this->gid == 1956){?>
	if (_address == '' || _address == '默认地址' || _address == '收货地址')
	{
		mui.alert('请填写收货地址', '提示信息');
		return false;
	}
	<?php }?>
