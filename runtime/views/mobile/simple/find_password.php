...
document.getElementById('sendcode').addEventListener('tap', function(){
    ...
    if ( s_count == default_time && send_status ){
        mui.get('<?php echo IUrl::creatUrl("/simple/send_message_mobile");?>', {mobile: mobile, username: username}, function(res){
            if(res == 'success'){
                updateState();
...
document.getElementById('find').addEventListener('tap', function(){
    ...
    mui.post('<?php echo IUrl::creatUrl("/simple/find_password_mobile_ajax");?>', {username: username, mobile: mobile, mobile_code: mobile_code}, function(json){
        if(json.status == 1){
