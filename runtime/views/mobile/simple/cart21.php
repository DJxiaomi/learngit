...
<form action='<?php echo IUrl::creatUrl("/simple/cart31");?>' method='post' name='order_form' id="order_form">
	...
	<input type="hidden" name="zuhe_id" value="<?php echo $this->zuhe_id;?>" />
	...
	<?php if(!$this->seller_tutor_id && !$this->user_tutor_id){?>
	...
	<div class="mui-card">
		<ul class="mui-table-view">
			<?php foreach($this->zuhe_list as $key => $item){?>
				<li class="mui-table-view-cell mui-media">
					<img class="mui-media-object mui-pull-left" src="<?php echo IUrl::creatUrl("/pic/thumb/img/".$item['logo']."/w/80/h/80");?>">
					<div class="mui-media-body">
						<?php echo isset($item['name'])?$item['name']:"";?> - <?php echo isset($item['use_times'])?$item['use_times']:"";?>课时
					<div class="qindanjg">价格：<span class="red3">&yen;<?php echo isset($item['max_price'])?$item['max_price']:"";?></span></div>
	...
	<div class="mui-card dqk_cart_info">
		<div class="mui-input-group">
			<li class="mui-table-view-cell">
				<div class="mui-media-body"><span class="t-left">原价</span><span class="t-right"><?php echo $this->old_price;?>元</span></div>
			</li>
			<li class="mui-table-view-cell">
				<div class="mui-media-body"><span class="t-left">优惠</span><span class="t-right">省<?php echo $this->discount;?>元</span></div>
			</li>
			<li class="mui-table-view-cell">
				<div class="mui-media-body"><span class="t-left">结算价</span><span class="red2 t-right" id="final_sum"><?php echo $this->market_count;?>元</span></div>
			</li>
		</div>
	</div>
...
order_chit = <?php if($this->order_chit >0){?><?php echo $this->order_chit;?><?php }else{?>0<?php }?>;
...
<?php if($this->is_show_tutor_detail){?>
...
jQuery(function(){
	...
	<?php if($this->statement == 4 && $this->user_tutor_info ){?>
	...
	<?php if($this->statement == 4 && $this->is_rehired){?>
	$('.mui-content-ks input').change(function(){
		...
		if ( !re.test(_num) )
		{
			...
			<?php if($this->seller_tutor_info){?>
				$.post('/simple/get_tutor_order_count', {seller_tutor_id: <?php echo $this->seller_tutor_id;?>, num: _num}, function(data){
			<?php }elseif($this->user_tutor_info){?>
				$.post('/simple/get_tutor_order_count', {user_tutor_id: <?php echo $this->user_tutor_id;?>, num: _num}, function(data){
			<?php }?>
				//console.log(data);
				if ( data == '0')
				{
	...
	$('input[name=coupon_nums]').change(function(){
		...
		} else if ( _val > _max_cprice )
		{
			...
			//console.log('ajax');
			var _url = '<?php echo IUrl::creatUrl("/site/get_order_chit_ajax/cprice/@cprice@/stime/1/statement/@stm@/id/@id@/type/@type@/num/@num@");?>';
			_url = _url.replace('@cprice@', _val);
			_url = _url.replace('@stm@', _stm );
			_url = _url.replace('@id@', <?php echo $this->gid;?> );
			_url = _url.replace('@type@', '<?php echo $this->type;?>' );
			_url = _url.replace('@num@', '<?php echo $this->num;?>' );
