...
<?php $list = seller_class::get_goods_list_by_seller_id($this->seller['seller_id']);?>
<article class="module width_full">
	<header>
		<h3 class="tabs_involved">商品列表</h3>
		<ul class="tabs">
			<li><input type="button" class="alt_btn" onclick="filterResult();" value="检索" /></li>
			<li><input type="button" class="alt_btn" onclick="window.location.href='<?php echo IUrl::creatUrl("/seller/goods_edit_1");?>';" value="添加商品" /></li>
      ...
      <li><input type="button" class="alt_btn" onclick="goodsSetting();" value="批量设置" /></li>
			<li style="display:none;"><input type="button" class="alt_btn" onclick="window.open('<?php echo IUrl::creatUrl("/seller/goods_report/?".$searchParam."");?>');" value="导出Excel" /></li>
		</ul>
	</header>
	...
	<form action="<?php echo IUrl::creatUrl("/seller/goods_del");?>" method="post" name="goodsForm">
		<table class="tablesorter" cellspacing="0">
			...
			<tbody>
				...
				<?php $where='seller_id='.$this->seller['seller_id'];?>
				<?php $query = new IQuery("goods as go");$query->page = "$page";$query->pagesize = "20";$query->where = "$where";$items = $query->find(); foreach($items as $key => $item){?>
				<tr>
					...
					<td>
						<?php echo $cname = seller_class::get_catname_by_goods_id($item[id]);?>
					</td>
					...
					<td>
						<a href="<?php echo IUrl::creatUrl("/seller/goods_edit_1/id/".$item['id']."");?>"><img src="<?php echo $this->getWebSkinPath()."images/main/icn_edit.png";?>" title="编辑" /></a>
