...
<article class="module width_full">
	...
	<table class="tablesorter" cellspacing="0">
		...
		<tbody>
			...
			<?php $query = new IQuery("comment as c");$query->join = "left join goods as goods on c.goods_id = goods.id left join user as u on c.user_id = u.id";$query->fields = "c.*,u.username,goods.id as goods_id,goods.name as goods_name";$query->page = "$page";$query->where = "c.status = 1 and goods.seller_id = $seller_id and $where";$query->order = "c.id desc";$items = $query->find(); foreach($items as $key => $item){?>
			<tr>
