	...
	function relationCallBack(content,regimentImg)
	{
		if(content)
		{
			...
			var html = '<tr><th>商品名称：</th><td>'+content['data']['name']+'</td></tr>'
					  ...
					  +'<tr><th>每人团购最小量：</th><td><input type="text" class="small" name="limit_min_count" pattern="^[1-9]\\d*$" alt="填写数字" />，<label>限制每个用户购买的最小数量，少于这个数量则无法购买</label></td></tr>'
					  +'<tr><th>每人团购最大量：</th><td><input type="text" class="small" name="limit_max_count" pattern="^[1-9]\\d*$" alt="填写数字" />，<label>限制每个用户购买的最大数量，大于这个数量则无法购买</label></td></tr>';
