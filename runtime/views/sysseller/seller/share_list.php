...
<?php $where = Util::search(IReq::get('search'));$join = isset($searchArray['search']['ce.category_id=']) ? " left join category_extend as ce on ce.goods_id = go.id " : ""?>
	...
	<form action="<?php echo IUrl::creatUrl("/seller/goods_copy");?>" method="post" name="goodsForm">
		<table class="tablesorter" cellspacing="0">
			...
			<tbody>
				...
				<?php $goodsObject = new IQuery("goods as go");$goodsObject->where = "go.is_share = 1 and go.is_del = 0 and go.seller_id = 0 and $where";$goodsObject->page = "$page";$goodsObject->join = "$join";$goodsObject->fields = "distinct go.id, go.*";$items = $goodsObject->find(); foreach($items as $key => $item){?>
				<tr>
