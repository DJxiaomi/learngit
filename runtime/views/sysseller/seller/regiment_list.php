	...
	<form method='post' action='<?php echo IUrl::creatUrl("/seller/regiment_del");?>'>
		<table class="tablesorter" cellspacing="0">
			...
			<tbody>
				...
				<?php $query = new IQuery("regiment");$query->where = "seller_id = $seller_id";$query->order = "sort asc";$query->page = "$page";$items = $query->find(); foreach($items as $key => $item){?>
				<tr>
					...
					<td>
						<a href='<?php echo IUrl::creatUrl("/seller/regiment_edit/id/".$item['id']."");?>'>
							<img src="<?php echo $this->getWebSkinPath()."images/main/icn_edit.png";?>" alt="修改" title="修改" />
						</a>

						<a href='javascript:void(0)' onclick="delModel({link:'<?php echo IUrl::creatUrl("/seller/regiment_del/id/".$item['id']."");?>'});">
							<img src="<?php echo $this->getWebSkinPath()."images/main/icn_del.png";?>" alt="删除" title="删除" />
						</a>
					</td>
				</tr>
				<?php }?>
