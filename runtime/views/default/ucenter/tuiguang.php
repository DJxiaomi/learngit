...
<body class="index">
	...
	<div class="<?php if( ($this->getId() == 'site' && $_GET['action'] == 'index') || ($this->getId() == 'site' && $_GET['action'] == '') || ($this->getId() == 'site' && $_GET['action'] == 'intro')){?>module_content_index<?php }else{?>module_content<?php }?>">
		<div class="main f_r">
	<div class="new-box">
        <div class="nt"><i class="icon-group"></i>推广记录</div>
        <table class="new-table">
            <tr><th>您推广的商户</th><th>当月销售额</th></tr>
            <?php $page= (isset($_GET['page'])&&(intval($_GET['page'])>0))?intval($_GET['page']):1;?>
			<?php $user_id = $this->user['user_id']?>
			<?php $getTuiGuangList = Api::run('getTuiGuangList',$user_id)?>
			<?php foreach($getTuiGuangList->find() as $key => $item){?>
			<tr>
			<td><?php echo isset($item['seller_name'])?$item['seller_name']:"";?></td>
			<td>
			<?php if(Lexiangshenghuo::getTuiGuangNum($item['id'])>0){?>
			<?php echo Lexiangshenghuo::getTuiGuangNum($item['id']);?>
			<?php }else{?>
			0.00
			<?php }?>
			</td>
			...
			<?php }?>
            <tfoot><tr><td colspan="2" class="t_l"><?php echo $getTuiGuangList->getPageBar();?></td></tr></tfoot>
        </table>
    </div>
</div>
