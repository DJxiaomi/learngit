...
<body class="index">
	...
	<div class="<?php if( ($this->getId() == 'site' && $_GET['action'] == 'index') || ($this->getId() == 'site' && $_GET['action'] == '') || ($this->getId() == 'site' && $_GET['action'] == 'intro')){?>module_content_index<?php }else{?>module_content<?php }?>">
		<link href="<?php echo $this->getWebSkinPath()."css/ucenter_article_list.css";?>" rel="stylesheet" type="text/css" />
<script language="javascript">
  var _del_url = "<?php echo IUrl::creatUrl("/ucenter/tutor_del/id");?>";
  var _auth_url = '<?php echo IUrl::creatUrl("/ucenter/authentication/back/1");?>';
  var _publish_url = '<?php echo IUrl::creatUrl("/ucenter/tutor_publish/id/@id@");?>';
  var _is_auth = <?php echo isset($is_auth)?$is_auth:"";?>;
</script>

<div class="main f_r">
    <div class="main f_r">
        <div class="new-box">
            <div class="nt"><i class="icon-file"></i>我的家教<span><a href="<?php echo IUrl::creatUrl("/ucenter/tutor_add");?>" class="blue">发布家教信息</a></span></div>
            ...
            <table class="new-table">
                ...
                <?php if($tutor_list){?>
                  ...
                  <tr>
                      <td></td>
                      <td><?php echo  get_gender_title($item['gender']);?></td>
                      <td><?php echo category_class::get_category_title($item['grade_level']);?></td>
                      <td><?php echo category_class::get_category_title($item['grade']);?></td>
                      <td><?php echo isset($item['lastest_scores'])?$item['lastest_scores']:"";?></td>
                      <td><?php echo isset($item['lowest_reward'])?$item['lowest_reward']:"";?></td>
					  <td><?php echo isset($item['expected_scores'])?$item['expected_scores']:"";?></td>
                      <td><?php echo tutor_class::get_tutor_status_title($item['status'], $item['is_publish']);?></td>
                      <td><?php echo date('Y-m-d',$item['create_time']);?></td>
                      <td>
                        <a href="<?php echo IUrl::creatUrl("/ucenter/tutor_add/id/".$item['id']."");?>" title="修改"><img class="operator" src="/views/sysdefault/skin/default/images/admin/icon_edit.gif" alt="修改"></a>
                        &nbsp; <a href="javascript:void(0);" _id="<?php echo isset($item[id])?$item[id]:"";?>" class="del_btn" title="删除"><img class="operator" src="/views/sysdefault/skin/default/images/admin/icon_del.gif" alt="删除"></a>
                        <?php if($item['contract_addr']){?><a href="<?php echo IUrl::creatUrl("")."".$item['contract_addr']."";?>" target="_blank">查看合同</a><?php }?>
                      </td>
                  </tr>
                  <?php }?>
                  <tfoot><tr><td colspan="10" class="t_l"><?php echo isset($page_info)?$page_info:"";?></td></tr></tfoot>
                <?php }else{?>
                <tr>
                  <td colspan="10">
                    暂时没有任何信息
...
<script type='text/javascript' src="<?php echo $this->getWebViewPath()."javascript/ucenter_tutor_list.js";?>"></script>
