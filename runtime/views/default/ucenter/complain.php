...
<body class="index">
    ...
    <table class="list_table m_10" width="100%" cellpadding="0" cellspacing="0">
		...
		<?php foreach($querySuggestionList->find() as $key => $item){?>
            <tr>
                <td class="t_l"><?php echo isset($item['title'])?$item['title']:"";?></td>
                <td><?php echo isset($item['time'])?$item['time']:"";?></td>
                <td>
                	<?php if($item['re_content']!=''){?>
                	<span class="green bold">已回复</span>
                	<?php }else{?>
                	<span>未回复</span>
                	<?php }?>
                </td>
                <td><a class="blue bold" href="javascript:void(0)" onclick='show_msg(this,<?php echo JSON::encode($item);?>)'>查看</a></td>
            </tr>
        <?php }?>
        <tr id='show_msg' class="show" style="display:none">
            <td colspan="4">
				...
				<span id='re_info'>
					<hr class="dashed" />
					<b class="orange"><?php echo $this->_siteConfig->name;?></b>
					在 [<span class="gray" id='re_time'></span>] 回复:<p class="gray indent mt_10" id='re_content'></p>
				</span>
			</td>
        </tr>
        </tbody>
