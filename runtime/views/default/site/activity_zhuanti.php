...
?>
<link href="<?php echo $this->getWebSkinPath()."css/discount.css";?>" rel="stylesheet" type="text/css" />

<div class="free_class_content">
  <div class="free_class_hd">
    <img src="<?php echo $this->getWebSkinPath()."images/discount_img_1.jpg";?>" />
  </div>
  <div class="free_class_bd">
    <?php if($list){?>
    <ul>
      <?php foreach($list as $key => $item){?>
      <li class="<?php if($item['limit_end_time'] > 0 && $item['limit_end_time'] <= $time){?>t2<?php }else{?>t1<?php }?>">
      	<a href="<?php echo IUrl::creatUrl("/site/products/id/".$item['id']."");?>">
            <div class="class_image">
              <img  class="lazy" data-original="<?php echo IUrl::creatUrl("")."".$item['img']."";?>" />
            </div>
            <div class="class_action">
              <p class="tt"><?php echo isset($item['name'])?$item['name']:"";?></p>
              <p class="detail"><?php echo isset($item['goods_brief'])?$item['goods_brief']:"";?></p>
            </div>
