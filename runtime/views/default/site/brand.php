...
<body class="index">
	...
	<div class="<?php if( ($this->getId() == 'site' && $_GET['action'] == 'index') || ($this->getId() == 'site' && $_GET['action'] == '') || ($this->getId() == 'site' && $_GET['action'] == 'intro')){?>module_content_index<?php }else{?>module_content<?php }?>">
		<link href="<?php echo $this->getWebSkinPath()."css/brand.css";?>" rel="stylesheet" type="text/css" />
...
<div class="wrapper clearfix container_2">
	<div class="sidebar f_l">
		<div class="box_2 m_10">
			<div class="title" color="red">学校分类</div>
			<div class="content">
				<ul class="brand_list">
	...
	<div class="sidebar f_l">
		<table id="index_category" class="sort_table m_10" width="100%" >
			...
			<tr>
				<th><a href="<?php echo IUrl::creatUrl("/site/brand/id/".$category_id."/cid/".$second['id']."");?>"><?php echo isset($first['name'])?$first['name']:"";?></a></th>
				<td>
					<?php foreach(Api::run('getCategoryByParentid',array('#parent_id#',$first['id'])) as $key => $second){?>
					<a href="<?php echo IUrl::creatUrl("/site/brand/id/".$category_id."/cid/".$second['id']."");?>"><?php echo isset($second['name'])?$second['name']:"";?></a>
					<?php }?>
				</td>
			</tr>
			<?php }?>
	...
	<div class="main f_r">
			...
			<div class="cont">
				<ul class="piclist_2">
					...
					<li class="intro_goods1">
						<div class="cont_img">
							<a class="pic" href="<?php echo IUrl::creatUrl("/site/brand_zone/id/".$item['id']."");?>">
								<img class="lazy" data-original="<?php echo IUrl::creatUrl("")."".$item['logo']."";?>"/>
							</a>
						</div>
						<div class="intro1">
					    <a href="<?php echo IUrl::creatUrl("/site/brand_zone/id/".$item['id']."");?>">
								<p class="intro_title"><?php echo isset($item['name'])?$item['name']:"";?></p>
					      <p class="intro_key"><?php echo strip_tags($item['description']);?></p>
								<p class="intro_short"><?php echo isset($item['shortname'])?$item['shortname']:"";?></p>
								<p class="intro_address"><?php echo isset($item['address'])?$item['address']:"";?></p>
							</a>
						</div>
					</li>
					<?php }?>
		...
		<?php }else{?>
			...
			<div class="cont">
				<ul class="piclist_2">
				   <?php if(!$brand_list){?>
					<div class="cont_none">对不起 没有找到相关商品</div>
				   <?php }?>
					<?php foreach($brand_list as $key => $item){?>
					<?php $temid=$brand_list['id']?>
					<li class="intro_goods1">
						<div class="cont_img">
							<a class="pic" href="<?php echo IUrl::creatUrl("/site/brand_zone/id/".$item['id']."");?>">
								<img class="lazy" data-original="<?php echo IUrl::creatUrl("")."".$item['logo']."";?>" />
						...
						<div class="intro1">
							<p class="intro_title">
								<a href="<?php echo IUrl::creatUrl("/site/brand_zone/id/".$item['id']."");?>"><?php echo isset($item['name'])?$item['name']:"";?></a>
							</p>
				      <p class="intro_key"><?php echo strip_tags($item['description']);?></p>
							<p class="intro_short"><?php echo isset($item['shortname'])?$item['shortname']:"";?></p>
							<p class="intro_address"><?php echo isset($item['address'])?$item['address']:"";?></p>
