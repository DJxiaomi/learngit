    ...
    <div class="card_cont">
      <div class="card-body student_info">
        <div class="">
            ...
            <div>
                <div class="s_left">
                    <img src="<?php echo IUrl::creatUrl("")."".$tutor_info['head_icon']."";?>" alt="">
                </div>
                <div class="s_right">
                    <span class="name"><?php echo isset($tutor_info['true_name'])?$tutor_info['true_name']:"";?></span>
                    <span class="sex"><?php echo get_gender_title($tutor_info['gender']);?></span>
                    <span class="kc">需补课程：<strong><?php echo category_class::get_category_title($tutor_info['grade']);?></strong></span>
                </div>
                <div class="cb"></div>
            </div>
            <p>性别：<span><?php echo get_gender_title($tutor_info['gender']);?></span></p>
            <p>年龄：<span><?php echo $tutor_info['age'];?>岁</span></p>
            <p>补课类目：<span><?php echo category_class::get_category_title($tutor_info['grade_level']);?> <?php echo category_class::get_category_title($tutor_info['grade']);?></span></p>
            <?php if($tutor_info['lastest_scores'] > 0){?><p>最近一次考分：<span><?php echo $tutor_info['lastest_scores'];?>分</span></p><?php }?>
            <?php if($tutor_info['expected_scores'] > 0){?><p>期望的考分：<span><?php echo $tutor_info['expected_scores'];?>分</span></p><?php }?>
            <?php if($tutor_info['lowest_reward'] > 0){?><p>最低的报酬：<span><?php echo $tutor_info['lowest_reward'];?></span>元/时</p><?php }?>
            <?php if($tutor_info['highest_reward'] > 0){?><p>最高的报酬：<span><?php echo $tutor_info['highest_reward'];?></span>元/时</p><?php }?>
            <?php if($tutor_info['expected_hours'] > 0){?><p>预计补课的课时：<span><?php echo $tutor_info['expected_hours'];?></span>时</p><?php }?>
            <?php if($tutor_info['teaching_time']){?><p>预计补课的时间：<span>每<?php foreach($tutor_info['teaching_time'] as $key => $item){?><?php if(!$key){?><?php echo isset($teaching_time_arr[$item['time1']])?$teaching_time_arr[$item['time1']]:"";?><?php }else{?>、<?php echo isset($teaching_time_arr[$item['time1']])?$teaching_time_arr[$item['time1']]:"";?><?php }?><?php }?></span></p><?php }?>
            <?php if($tutor_info['is_provide_transportation_fee']){?><p>是否提供交通费用：<span><?php if($tutor_info['is_provide_transportation_fee']){?>是<?php }else{?>否<?php }?></span></p><?php }?>
            <?php if($tutor_info['is_provide_repast']){?><p>是否提供就餐：<span><?php if($tutor_info['is_provide_repast']){?>是<?php }else{?>否<?php }?></span></p><?php }?>
            <p>地址：<span><?php echo area::getName($tutor_info['region_id']);?> <?php echo isset($tutor_info['address'])?$tutor_info['address']:"";?> <?php echo isset($tutor_info['address2'])?$tutor_info['address2']:"";?></span></p>
    ...
    <div class="card_sidebar">
        <div class="card_sidebar">
          ...
          <div class="tutor_menu" style="width: 334px;">
              <a href="<?php echo IUrl::creatUrl("/site/tutor");?>">我是学生</a>
              <a href="<?php echo IUrl::creatUrl("/site/user_tutor_list");?>" class="active">我是家教</a>
          </div>
            ...
            <div class="sidebar_box">
                ...
                <div class="t_list_sbox">
                    <ul>
                      ...
                      <li><a href="<?php echo IUrl::creatUrl("/site/tutor_info/id/".$item['id']."");?>" class="t_simg"><img src="<?php echo IUrl::creatUrl("")."".$item['logo']."";?>"></a>
...
$(document).ready(function() {
    ...
    $('.btn-zx').click(function(){
        location.href = '/seller2/cart2/id/<?php echo isset($id)?$id:"";?>';
    })
});
