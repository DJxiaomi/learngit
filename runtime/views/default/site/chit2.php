...
<body class="index">
	...
	<div class="<?php if( ($this->getId() == 'site' && $_GET['action'] == 'index') || ($this->getId() == 'site' && $_GET['action'] == '') || ($this->getId() == 'site' && $_GET['action'] == 'intro')){?>module_content_index<?php }else{?>module_content<?php }?>">
		<link href="<?php echo $this->getWebSkinPath()."css/chit.css";?>" rel="stylesheet" type="text/css" />
<div class="main f_r">

  <div class="new-box">
        <div class="nt" style="padding: 10px 0;">学习券购买</div>
        <table class="new-order-table">
            ...
            <?php foreach($zuhe_list as $key => $item){?>
            <tbody>
              <tr class="tr-td">
                <td>
                  <div class="q">
                    <div style="position:absolute;top:0;left:10px;">
                      <div class="qbd"><?php echo isset($item['name'])?$item['name']:"";?></div>
                      <div class="qhd" style="font-size: 22px;">&yen;<?php echo isset($item['price'])?$item['price']:"";?></div>

                    </div>
                  </div>

                  <div class="i">
                    <div class="hd"><?php echo isset($item['name'])?$item['name']:"";?></div>
                    <div class="bd">0</div>
                  </div>
                </td>
                <td>代金券组合</td>
                ...
                <td><a href="<?php echo IUrl::creatUrl("/simple/cart21/id/5280/num/1/type/product/statement/2/stime/1/ischit/1/zuhe_id/".$item['id']."");?>">立即购买</a></td>
                </tr>
            </tbody>
            <?php }?>
            ...
            <?php foreach($this->chit_list as $key => $item){?>
            <tbody>
              <tr class="tr-td">
                <td>
                  <div class="q">
                    <div style="position:absolute;top:0;left:10px;">
                      <div class="qbd">满<?php echo isset($item['amount'])?$item['amount']:"";?>元可用</div>
                      <div class="qhd" style="font-size: 22px;">&yen;<?php echo isset($item['max_order_chit'])?$item['max_order_chit']:"";?></div>
                      
                    </div>
                  </div>

                  <div class="i">
                    <div class="hd"><?php echo isset($item['seller_info']['shortname'])?$item['seller_info']['shortname']:"";?></div>
                    <div class="bd"><?php echo isset($item['limitinfo'])?$item['limitinfo']:"";?></div>
                  </div>
                </td>
                <td><?php if($item['type'] == 1){?>通用券<?php }else{?>赠送券<?php }?></td>
                <td>￥<?php echo isset($item['max_price'])?$item['max_price']:"";?></td>
                <td><?php echo isset($item['limitnum'])?$item['limitnum']:"";?></td>
                <td>
                  <?php if(isset($item['pid'])){?>
                    <a href="<?php echo IUrl::creatUrl("/simple/cart2/id/".$item['pid']."/num/1/type/product/statement/2/stime/1/dprice/".$item['max_price']."");?>">立即购买</a>
                  <?php }else{?>
                    <a href="<?php echo IUrl::creatUrl("/site/chit_show/id/".$item['id']."");?>">查看详情</a>
                  <?php }?>
                </td>
                </tr>
            </tbody>
            <?php }?>
            <tfoot><tr><td colspan="11" class="t_l"><?php echo isset($page_info)?$page_info:"";?></td></tr></tfoot>
        </table>
    </div>
</div>
...
function buy_now_ding_card_zuhe(id) {
	var buyNums  = 1;
	var url = '<?php echo IUrl::creatUrl("/simple/cart21/id/5280/num/@buyNums@/type/product/statement/2/ischit/1/zuhe_id/@zuhe_id@");?>';
		url = url.replace('@zuhe_id@',id).replace('@buyNums@',buyNums);
	var _input_stime = 1;
	url += '/stime/' + _input_stime;
