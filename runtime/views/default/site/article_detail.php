...
<link href="<?php echo $this->getWebSkinPath()."css/article.css";?>" rel="stylesheet" type="text/css" />
        ...
        <div class="Wrap clearfix">
		   ...
		   <div class="sideBox fl mr20 clearfix">
			...
			<div class="mainBox fr clearfix">
				<div class="website">
					<a href="<?php echo IUrl::creatUrl("/site/index");?>" target="_blank">首页</a>&gt;
					<?php foreach($article_info['path'] as $key => $item){?>
					<a href="<?php echo IUrl::creatUrl("/site/article/cid/".$item['id']."");?>" target="_blank"><?php echo isset($item['name'])?$item['name']:"";?></a>
					<?php }?>
				</div>
				<div class="artCon clearfix">
					<div class="art-tips clearfix">
						<h2 class=""><?php echo isset($article_info['title'])?$article_info['title']:"";?></h2>
						<p class="c9 f12"><span class="blue mr20"><?php if($article_info['user_name']){?><?php echo isset($article_info['user_name'])?$article_info['user_name']:"";?><?php }else{?>乐享生活<?php }?></span><?php echo  date('Y年m月d日 H:i:s', strtotime( $article_info['create_time']));?></p>
						<p class="c9 f12">声明：本文由入驻乐享公众平台的作者撰写，除乐享官方账号外，观点仅代表作者本人，不代表乐享立场。</p>
						<div class="text-share"><span class="shareit f12 orange"><?php echo isset($article_info['views'])?$article_info['views']:"";?></span><span class="comment"><a href="#reply"><?php echo isset($article_info['reply_count'])?$article_info['reply_count']:"";?></span></a></div>
					</div>
					<?php if($page == 1){?>
				...
				<div class="disBox clearfix mt20">
					<?php if($reply_list){?>
					<ul>
						<?php foreach($reply_list as $kk => $item){?>
						<li class="discussCon clearfix">
							<div class="headImg fl">
								<a href="javascript:void(0);" class="headImg-yh">
									<img src="<?php echo IUrl::creatUrl("")."".$user_list[$item['user_id']]['head_ico']."";?>" onerror="this.src='<?php echo $this->getWebSkinPath()."images/front/user_ico.gif";?>'" width="80" height="80"/>
								</a>
								<a href="javascript:void(0);" class="headName"><?php echo isset($user_list[$item['user_id']]['username'])?$user_list[$item['user_id']]['username']:"";?></a></div>
							<div class="dis-text clearfix fr"><p><?php echo isset($item['content'])?$item['content']:"";?></p></div>
							<i class="disTime c9 f12"><?php echo  ( $page - 1 ) * $page_size + $kk + 1;?>楼&nbsp;<?php echo  date('Y-m-d H:i:s', $item['add_time']);?><a href="" class="c9 f12 ml20" style="display: none;">赞(20)</a></i>
						</li>
						<?php }?>
					</ul>
					<?php }?>
			...
			<div class="dis-conbox clearfix">
				<form method="POST" action="<?php echo IUrl::creatUrl("/site/article_reply");?>" id="form_reply">
					...
					验证码：<input type="text" id="captcha" name="captcha" class="normal small" /><img src="<?php echo IUrl::creatUrl("/simple/getCaptcha");?>" onclick="this.src=this.src+'?time='+Math.random()"  id="captchaImg">
					<input type="button" class="submit-btn" onclick="this.src=<?php echo IUrl::creatUrl("/simple/getCaptcha");?>" value="提交" placeholder=""/>
					<input type="hidden" name="article_id" value="<?php echo $this->article_id;?>" />
				</form>
			</div>
			<?php }else{?>
				<div class="dis-conbox pls_login">
					<a href="<?php echo IUrl::creatUrl("/simple/login");?>" class="blue">登录</a>    进行评价
				</div>
			<?php }?>
		</div>
