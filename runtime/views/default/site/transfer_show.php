...
<link href="<?php echo $this->getWebSkinPath()."css/transfer_show.css";?>" rel="stylesheet" type="text/css" />

<div class="position"><span>您当前的位置：</span><a href="<?php echo IUrl::creatUrl("");?>">首页</a> » <?php if($transfer['transfername']){?><?php echo isset($transfer['transfername'])?$transfer['transfername']:"";?><?php }else{?>转让-<?php echo isset($transfer['goods']['name'])?$transfer['goods']['name']:"";?><?php }?></div>

<div class="wrapper clearfix">
	<div class="summary">
	<form action="<?php echo IUrl::creatUrl("/simple/trasnferorder");?>" method="post">
		<h2 class=""><?php if($transfer['transfername']){?><?php echo isset($transfer['transfername'])?$transfer['transfername']:"";?><?php }else{?>转让-<?php echo isset($transfer['goods']['name'])?$transfer['goods']['name']:"";?><?php }?></h2>

		<!--基本信息区域-->
		<ul>
			<?php if($transfer['goods_id']){?>
			<li>
				<span class="f_r light_gray">课程编号：<label id="data_goodsNo"><?php echo $transfer['goods']['goods_no']?$transfer['goods']['goods_no']:$transfer['goods']['id'];?></label></span>
			</li>
			<?php }?>
			<li>转让人：<?php echo isset($transfer['username'])?$transfer['username']:"";?></li>
			<li>总课时：<?php echo isset($transfer['totaltime'])?$transfer['totaltime']:"";?></li>
			<li>剩余课时：<?php echo isset($transfer['minustime'])?$transfer['minustime']:"";?></li>
			<?php if($item['istalk']){?>
			<li>已与商家沟通好</li>
			<?php }?>
			<li>原价：<b class="price red2" style=" text-decoration: line-through;"><span class="f30">￥<?php if($transfer['sellerprice']){?><?php echo isset($transfer['sellerprice'])?$transfer['sellerprice']:"";?><?php }else{?><?php echo isset($transfer['goods']['sell_price'])?$transfer['goods']['sell_price']:"";?><?php }?></span></b></li>
			<li>转让价：<b class="price red2"><span class="f30">￥<?php echo isset($transfer['transprice'])?$transfer['transprice']:"";?></span></b></li>
			<li id='priceLi'></li>
			<?php if($transfer['seller']['id']){?>
			<!--商家信息 开始-->
			<li>商家：<a class="blue-font" href="<?php echo IUrl::creatUrl("/site/seller/id/".$transfer['seller']['id']."");?>" target="_blank"><?php echo isset($transfer['seller']['true_name'])?$transfer['seller']['true_name']:"";?></a></li>
			<li>联系电话：<?php echo isset($transfer['seller']['phone'])?$transfer['seller']['phone']:"";?></li>
			<li>所在地：<?php echo join(' ',area::name($transfer['seller']['province'],$transfer['seller']['city'],$transfer['seller']['area'])).' '.$transfer['seller']['address'];?></li>
			<?php }?>
			<!--商家信息 结束-->
		</ul>
		...
		<!--购买区域-->
		<div class="current">
		    <input type="hidden" name="goods_id" value="<?php echo isset($transfer['goods']['id'])?$transfer['goods']['id']:"";?>">
		    <input type="hidden" name="goods_name" value="<?php echo isset($transfer['goods']['name'])?$transfer['goods']['name']:"";?>">
		    <input type="hidden" name="selleruid" value="<?php echo isset($transfer['uid'])?$transfer['uid']:"";?>">
		    <input type="hidden" name="sellerid" value="<?php echo isset($transfer['seller_id'])?$transfer['seller_id']:"";?>">
		    <input type="hidden" name="transferid" value="<?php echo isset($transfer['id'])?$transfer['id']:"";?>">
	...
	<div class="preview">
		...
		<ul id="thumblist" class="pic_thumb">
			<?php if($transfer['transferpic']){?>
			<li>
				<a href='javascript:void(0);' rel="{gallery:'goodsPhoto',smallimage:'<?php echo IUrl::creatUrl("")."";?><?php echo Thumb::get($transfer['transferpic'],435,435);?>',largeimage:'<?php echo IUrl::creatUrl("")."".$transfer['transferpic']."";?>'}">
					<img src='<?php echo IUrl::creatUrl("/pic/thumb/img/".$transfer['transferpic']."/w/80/h/80");?>' width="80px" height="80px" />
				</a>
			</li>
			<?php }else{?>
			<?php foreach($transfer['goods']['photo'] as $key => $item){?>
			<li>
				<a href='javascript:void(0);' rel="{gallery:'goodsPhoto',smallimage:'<?php echo IUrl::creatUrl("")."";?><?php echo Thumb::get($item['img'],435,435);?>',largeimage:'<?php echo IUrl::creatUrl("")."".$item['img']."";?>'}">
					<img src='<?php echo IUrl::creatUrl("/pic/thumb/img/".$item['img']."/w/80/h/80");?>' width="80px" height="80px" />
				</a>
			</li>
			<?php }?>
			<?php }?>
		...
		<div name="showBox">
			...
			<div>
				<ul class="saleinfos m_10 clearfix">
					<li>课程：<?php echo isset($transfer['goods']['name'])?$transfer['goods']['name']:"";?></li>
					<?php if($transfer['goods']['category']){?><li>课程分类：<?php echo isset($transfer['goods']['category'])?$transfer['goods']['category']:"";?></li><?php }?>
					<?php if(($transfer['goods']['attribute'])){?>
					<?php foreach($transfer['goods']['attribute'] as $key => $item){?>
					<li><?php echo isset($item['name'])?$item['name']:"";?>：<?php echo isset($item['attribute_value'])?$item['attribute_value']:"";?></li>
					<?php }?>
					<?php }?>
				</ul>

				<?php if(isset($transfer['goods']['content']) && $transfer['goods']['content']){?>

				<div class="salebox">

					<strong class="saletitle block">课程简介：</strong>

					<p class="saledesc"><?php echo isset($transfer['goods']['content'])?$transfer['goods']['content']:"";?></p>

				</div>

				<?php }?>

				<?php if(isset($transfer['goods']['focus']) && $transfer['goods']['focus']){?>

				<div class="salebox">

					<strong class="saletitle block">教学大纲：</strong>

					<p class="saledesc"><?php echo isset($transfer['goods']['focus'])?$transfer['goods']['focus']:"";?></p>

				</div>

				<?php }?>

				<?php if(isset($transfer['goods']['awards']) && $transfer['goods']['awards']){?>

				<div class="salebox">

					<strong class="saletitle block">获奖荣誉：</strong>

					<p class="saledesc"><?php echo isset($transfer['goods']['awards'])?$transfer['goods']['awards']:"";?></p>

				</div>

				<?php }?>

				<?php if(isset($transfer['goods']['expense_list']) && $transfer['goods']['expense_list']){?>

				<div class="salebox">

					<strong class="saletitle block">费用说明：</strong>

					<p class="saledesc"><?php echo isset($transfer['goods']['expense_list'])?$transfer['goods']['expense_list']:"";?></p>

				</div>

				<?php }?>

				<?php if(isset($transfer['goods']['course_imgs']) && $transfer['goods']['course_imgs']){?>

				<div class="salebox">

					<strong class="saletitle block">课堂照片：</strong>
                   <?php foreach($transfer['goods']['course_imgs_arr'] as $key => $item){?>
					<p class="saledesc">
				...
				<?php if(isset($transfer['goods']['students_imgs']) && $transfer['goods']['students_imgs']){?>

				<div class="salebox">

					<strong class="saletitle block">学员展示：</strong>
                    <?php foreach($transfer['goods']['students_imgs_arr'] as $key => $item){?>
					<p class="saledesc">
                      <img src="/<?php echo isset($item)?$item:"";?>" />
                    </p>
                   <?php }?>
			...
			<div class="hidden">
				<div class="salebox">
					...
					<p class="saledesc"><?php echo isset($transfer['content'])?$transfer['content']:"";?></p>
...
<?php if(isset($transfer['goods']['photo']) && $transfer['goods']['photo']){?>

goodsSmallPic = "<?php echo IUrl::creatUrl("")."";?><?php echo Thumb::get($transfer['goods']['photo'][0]['img'],435,435);?>";

goodsBigPic   = "<?php echo IUrl::creatUrl("")."";?><?php echo isset($transfer['goods']['photo'][0][0]['img'])?$transfer['goods']['photo'][0][0]['img']:"";?>";
