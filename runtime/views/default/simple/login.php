...
<link href="<?php echo $this->getWebSkinPath()."css/font-awesome.min.css";?>" rel="stylesheet" type="text/css" />
<link href="<?php echo $this->getWebSkinPath()."css/login.css";?>" rel="stylesheet" type="text/css" />
...
<div class="new-loginbox clearfix">
	<div class="login-banner"></div>
	<div class="loginbox">
		<h3>用户登录</h3>
		<p class="tip" id="tip">请填写登陆信息</p>
		<p class="inbox clearfix">
        	<span><i class="icon-user"></i></span><input placeholder="用户名/邮箱/手机号" autocomplete="off" type="text" name="login_info" id="login_info" value="<?php echo ICookie::get('loginName');?>" tabindex="1">
        </p>
        <p class="inbox clearfix">
        	<span><i class="icon-lock"></i></span>
        	<input placeholder="请输入密码" type="password" id="password" name="password" tabindex="2">
        </p>
        <p class="clearfix">
        	<label style="cursor:pointer;"><input class="radio" type="checkbox" name="remember" value='1' />记住登录名</label>
        	<a class="forget" href="<?php echo IUrl::creatUrl("/simple/find_password");?>">忘记密码</a>
        </p>
        <p class="clearfix">
        	<input class="submit-btn" type="button" value="登 录" onclick="onlogin()" />
        </p>
        <p class="auth clearfix"  style="display:none;">
        	<?php foreach(Api::run('getOauthList') as $key => $item){?>

			<a href="javascript:oauthlogin('<?php echo isset($item['id'])?$item['id']:"";?>');"  style="float:left"><img width="20" src='<?php echo IUrl::creatUrl("")."".$item['logo']."";?>'/></a>

			<?php }?>
        </p>
        <p class="clearfix">
        	<a class="forget reg" href="<?php echo IUrl::creatUrl("/simple/reg");?>">立即注册</a>
