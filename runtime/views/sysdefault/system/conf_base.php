...
<div class="headbar">
	...
	<ul class='tab' name='conf_menu'>
		...
		<li><a href="javascript:select_tab('index_slide');">PC端幻灯设置</a></li>
		<li><a href="javascript:select_tab('index_slide_mobile');">手机端幻灯设置</a></li>
		<li><a href="javascript:select_tab('site_footer_conf');">站点底部信息</a></li>
		<li style="display:none;"><a href="javascript:select_tab('other_conf');">其他设置</a></li>
		<li style="display:none;"><a href="javascript:select_tab('mail_conf');">邮箱设置</a></li>
...
<div class="content_box">
	...
	<div class="content form_content">
				...
				<tr>
					...
					<td>
						...
						<table class='border_table' id='slide_box'>
							...
							<tr class='td_c'>
								<td><input type='text' name='slide_name[]' class='middle' value='<%=name%>' /></td>
								<td><input type='text' name='slide_url[]' class='middle' value='<%=url%>' /></td>
								<td>
									<%if(img){%>
		...
		<form action='<?php echo IUrl::creatUrl("/system/save_conf/form_index/index_slide_mobile");?>' method='post' name='index_slide_mobile' enctype="multipart/form-data">
			<table class='form_table'>
				<col width="150px" />
				<col />
				...
				<tr>
					...
					<td>
						...
						<table class='border_table' id='slide_box_mobile'>
							<col width="150px" />
							<col width="250px" />
							<col width="250px" />
							<col width="120px" />
							...
							<tfoot>
								<tr>
									<td colspan='4'>
										<button type='button' class='btn' onclick="add_slide_mobile();"><span>添加幻灯</span></button>
									</td>
								</tr>
							</tfoot>

							<script type='text/html' id='slideTrMobileTemplate'>
							<tr class='td_c'>
								<td><input type='text' name='slide_name[]' class='middle' value='<%=name%>' /></td>
								<td><input type='text' name='slide_url[]' class='middle' value='<%=url%>' /></td>
								<td>
		...
		<form action='<?php echo IUrl::creatUrl("/system/save_conf/form_index/other_conf");?>' method='post' name='other_conf' style='display:none;'>
			<table class="form_table">
				<colgroup>
					<col width="150px" />
					<col />
		...
		<form action='<?php echo IUrl::creatUrl("/system/save_conf/form_index/mail_conf");?>' method='post' name='mail_conf' style='display:none;'>
			<!--邮箱设置-->
			<table class="form_table">
				<colgroup>
					<col width="150px" />
...
$(function(){
	...
	//生成手机端幻灯片
	<?php if(isset($this->confRow['index_slide_mobile']) && $this->confRow['index_slide_mobile'] && $slide = unserialize($this->confRow['index_slide_mobile'])){?>
	var slideList = <?php echo JSON::encode($slide);?>;
	for(var index in slideList)
	{
		add_slide_mobile(slideList[index]);
	}
	<?php }else{?>
		add_slide_mobile();
	<?php }?>
...
function add_slide_mobile(data)
{
	var data = data ? data : {};
	var slideTrHtml = template.render('slideTrMobileTemplate',data);
	$('#slide_box_mobile tbody').append(slideTrHtml);
	var last_index = $('#slide_box tbody tr').size()-1;
	buttonInit(last_index,'#slide_box');
}
