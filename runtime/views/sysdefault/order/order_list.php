	...
	<div class="searchbar">
		<form action="<?php echo IUrl::creatUrl("/");?>" method="get" name="order_list">
			...
			<select name="search[pay_type]" class="auto">
				...
				<option value="1" <?php if($search['pay_type'] == 1){?>selected<?php }?>>第三课余额</option>
				<option value="14" <?php if($search['pay_type'] == 14){?>selected<?php }?>>微信</option>
				<option value="16" <?php if($search['pay_type'] == 16){?>selected<?php }?>>支付宝</option>
			</select>
			...
			<input type="checkbox" name="user_group" value="1" <?php if($user_group == 1){?>checked<?php }?>/>过滤员工订单&nbsp;
		...
		<table class="list_table">
			...
			<tbody>
				...
				<tr>
					...
					<td title="<?php echo isset($item['order_amount'])?$item['order_amount']:"";?>"><?php echo isset($item['order_amount'])?$item['order_amount']:"";?></td>
