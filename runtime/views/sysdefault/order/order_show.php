...
<div id="tab-1" name="table" style="display:none">
	<div class="content">
		<table class="list_table">
			...
			<thead>
				<tr>
					<th><?php if($brand_chit_list){?>短期课名称<?php }else{?>商品名称<?php }?></th>
					<th>商品原价</th>
					<th>实际价格</th>
					<th>商品数量</th>
					<th>小计</th>
			...
			<tbody>
				<!-- 短期课列表 -->
				<?php if($brand_chit_list){?>
					<?php foreach($brand_chit_list as $key => $item){?>
					<tr>
						<td>
							<?php echo isset($item['name'])?$item['name']:"";?> - <?php echo isset($item['shortname'])?$item['shortname']:"";?> (短期课)
						</td>
						<td>￥<?php echo isset($item['max_order_chit'])?$item['max_order_chit']:"";?></td>
						<td>￥<?php if(sizeof($brand_chit_list) > 1){?><?php echo isset($item['tc_price'])?$item['tc_price']:"";?><?php }else{?><?php echo isset($item['max_price'])?$item['max_price']:"";?><?php }?></td>
						<td>1</td>
						<td>￥<?php if(sizeof($brand_chit_list) > 1){?><?php echo isset($item['tc_price'])?$item['tc_price']:"";?><?php }else{?><?php echo isset($item['max_price'])?$item['max_price']:"";?><?php }?></td>
						<td>
						</td>
