		...
		<table class="list_table">
			...
			<tbody>
				...
				<tr>
					...
					<td><?php if($item['seller_info']['manual_category_id']){?><?php echo isset($category_list[$item['seller_info']['manual_category_id']]['name'])?$category_list[$item['seller_info']['manual_category_id']]['name']:"";?><?php }else{?>无<?php }?></td>
					<td><?php echo isset($item['max_price'])?$item['max_price']:"";?></td>
					<td><?php if($item['tc_price'] > 0){?><?php echo isset($item['tc_price'])?$item['tc_price']:"";?><?php }else{?>0<?php }?></td>
					<td><?php echo isset($item['max_order_chit'])?$item['max_order_chit']:"";?></td>
					<td><?php echo isset($item['commission'])?$item['commission']:"";?></td>
					<td><?php echo get_boolen_title($item['is_intro']);?></td>
					<td><?php echo get_boolen_title($item['is_top']);?></td>
					<td><?php if($item['is_del']){?>下架<?php }else{?>上架<?php }?></td>
					<td><?php echo isset($item['use_times'])?$item['use_times']:"";?>课时</td>
					<td><?php echo isset($item['each_times'])?$item['each_times']:"";?>课时</td>
					...
					<td>
						<a href='<?php echo IUrl::creatUrl("/brand/brand_chit_edit2/id/".$item['id']."/page/".$page."");?>'>
							<img class="operator" src="<?php echo $this->getWebSkinPath()."images/admin/icon_edit.gif";?>" alt="修改" title="修改" />
						</a>

						<a href='javascript:void(0)' onclick="delModel({link:'<?php echo IUrl::creatUrl("/market/brand_chit_del/id/".$item['id']."");?>'});">
				...
				<?php if($page_info){?>
				<tr>
					<td colspan="15"><?php echo isset($page_info)?$page_info:"";?></td>
				</tr>
				<?php }?>
			</tbody>
		</table>
